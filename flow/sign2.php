...
if (isset ($openid_cookie) && !empty ($openid_cookie)) { //先从COOKIE找
	$openid = $openid_cookie;

}
elseif (!empty ($session_user)) {

	$openid = get_database_user_openid($session_user); //再通过userid查询本地存的openid
}

if (empty ($openid)) { //以止都获取不到，就实时请求
	$get_code_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . APPID . '&redirect_uri=' . urlencode(THIS_URL . '/flow/sign.php') . '&response_type=code&scope=snsapi_base&state=#wechat_redirect';
	if (empty ($_GET['code'])) {
		...
		$openid = $data['openid'];
		if (OPENID_PREFIX) {
			setcookie(OPENID_PREFIX . 'openid', $openid);
		} //是否生成openid的session
	}
}

if (empty ($openid)) { //如果都获取不到就重新请求
	$error_num = $_SESSION['error_num'];
	if (empty ($error_num)) {
		$_SESSION['error_num'] = 1;
	} else {
		$_SESSION['error_num'] = $error_num +1;
	}
...
function get_database_user_openid($user_id) {
	...
	$result = mysql_query("SELECT " . USER_TO_WECHAT . " FROM " . USER_TABLE . " WHERE " . USER_ID . "=" . $user_id . " limit 1", $con);

	$row = mysql_fetch_assoc($result);
	if (!empty ($row)) {

		$result = mysql_query("SELECT " . OPENID . " FROM " . WECHAT_TABLE . " WHERE " . WECHAT_TO_USER . "='" . $row[USER_TO_WECHAT] . "' limit 1", $con);
		$row = mysql_fetch_assoc($result);
		return $row[OPENID];
