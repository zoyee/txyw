...
$logger = LoggerManager::getLogger('category.php');
...
$size = isset($_CFG['page_size'])  && intval($_CFG['page_size']) > 0 ? intval($_CFG['page_size']) : 1000;
...
//    $smarty->assign('categories',       get_categories_tree($cat_id)); // 分类树
    ...
    //下拉瀑布流式查询
    if(isset($_REQUEST['ajax_more']) && $_REQUEST['ajax_more'] == '1'){
...
//	$logger->debug(json_encode($goodslist));
    $smarty->assign('goods_list',       $goodslist);
    $smarty->assign('category',         $cat_id);
    $smarty->assign('script_name', 'category');
	$categiry_info = $db->getRow("select * from " . $ecs->table('category') . " where cat_id='$cat_id'");
	$smarty->assign('category_info', $categiry_info);
}

//下拉瀑布流式查询,返回json数据列表
if(isset($_REQUEST['ajax_more']) && $_REQUEST['ajax_more'] == '1'){
	foreach ($goodslist as $item){
		$prompt_ico = get_goods_special_icons($item['goods_id']);
		$prompt_html = "";
		if($prompt_ico){
			$prompt_html = '<a href="'.$item['url'].'"><div style="position:absolute;width:100%; padding-top:' .$GLOBALS["_CFG"]["special_cat_conf"]["top"] . ';left:' .$GLOBALS["_CFG"]["special_cat_conf"]["left"] . '">';
		}
		...
		$cart = "";
		if($item['is_agent']){
			$cart = '<a href="javascript:addToCart('.$item['goods_id'].');"><img src="'.'themes/' . $_CFG['template'].'/images/category/cart.png"/></a>';
		}

		$arr_more[] = array(
				'proImg-wrap' => '<a href="'.$item['url'].'" > <img class="card-cover" src="'.$config['site_url'].$item['goods_thumb'].'" alt="'.$item['name'].'"/>',
				'proTitle' => $item['name'],
				'proSubTitle' => $item['goods_guide'],
//				'proSubTitle' => '<a href="'.$item['url'].'" >'.$item['goods_attr'].'</a>',
				'proPrice' => empty($item['promote_price']) ? '<span class="price">'.$item['shop_price'].'</span>'  
							: '<span class="price">'.$item['promote_price'].'</span>',
 				'proService' => $cart,
				'proSales' => '销量：<span class="sales">'.$item['sales_count'].'</span>',
				'proIcons' => empty($item['watermark_img']) ? '' : '<img width="55" height="16" src="themes/'.$_CFG['template'].'/images/'.$item['watermark_img'].'.png" alt="" />',
				'prompt-ico' => $prompt_html
		);
	}
	$ret = json_encode($arr_more);
	exit ($ret);
} else {
	$catimgurl = "";
	$sql = "SELECT cat_img ".
	...
	$catimgrow = $db->getRow($sql);
	if($catimgrow){
		$catimgurl = $catimgrow['cat_img'];
	}
	$smarty->assign('catimgurl', $catimgurl);
	$smarty->assign('fullpage', 1);
	$smarty->assign('ajax_more', '1');
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id, prompt_ico FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, b.brand_name, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img,g.sales_volume_base, g.goods_guide ' .
                (empty($_SESSION[user_id]) ? "" : ", su.user_id ") .
            ...
            "left join " . $GLOBALS['ecs']->table('suppliers_user') . " as su on s.suppliers_id=su.suppliers_id and su.status=5 and su.user_id='$_SESSION[user_id]' ") .
            "WHERE $where $ext ORDER BY " . (empty($_SESSION[user_id]) ? "" : "su.user_id desc, ") . "$sort $order";
    $logger->debug($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['prompt_ico'] = get_goods_special_icons($row['goods_id']);

		$sql = "select attr_value from " . $GLOBALS['ecs']->table('goods_attr') . " where goods_id='$row[goods_id]' order by attr_id";
		...
		$arr[$row['goods_id']]['goods_attr'] = $attr_str;
		$arr[$row['goods_id']]['brand_name'] = $row['brand_name'];
		if($row['user_id']){
			$arr[$row['goods_id']]['is_agent'] = 1;
		}
    }
