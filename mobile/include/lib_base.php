...
function ecs_iconv($source_lang, $target_lang, $source_string = '')
{
    ...
    if ($chs === NULL)
    {
        require_once(ROOT_PATH . 'include/cls_iconv.php');
...
function ecs_geoip($ip)
{
    ...
    if ($fp === NULL)
    {
        $fp = fopen(ROOT_PATH . 'include/codetable/ipdata.dat', 'rb');
        if ($fp === false)
        {
...
function read_static_cache($cache_name)
{
    ...
    $cache_file_path = ROOT_PATH . 'data/static_caches/' . $cache_name . '.php';
