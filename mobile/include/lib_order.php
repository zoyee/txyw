...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('touch_shipping') . ' AS s, ' .
                ...
                $GLOBALS['ecs']->table('touch_area_region') . ' AS r ' .
...
function order_fee($order, $goods, $consignee)
{
    ...
    //查询不参与分销的商品列表
    $commission_unable_list = $GLOBALS['db']->getAll("select goods_id from " . $GLOBALS['ecs']->table('goods') . " where commission_able = 0");
    $commission_unable_arr = array();
    foreach($commission_unable_list as $row){
    	array_push($commission_unable_arr, $row['goods_id']);
    }
    $commission_unable_amount = 0;
//    $logger = LoggerManager::getLogger('lib_order.php');
//    $logger->debug('$commission_unable_list = ' . json_encode($commission_unable_list));
    ...
    foreach ($goods AS $val)
    {
		...
		//计算商品的
		/*青山老农新增加入订单商品分成金额*/
        //修改基数为实付金额，排除了红包、积分、余额抵扣的金额 by liuzhy 20160301
//         $fencheng = 0;
// 		if(!empty($val['fencheng'])){
// 			$total['fencheng']  += $val['fencheng'] * $val['goods_number'];
// 			$fencheng=$val['fencheng'] * $val['goods_number'];

// 		}else{
// 			$total['fencheng']  += $val['goods_price'] * $val['goods_number'];
// 			$fencheng=$val['goods_price'] * $val['goods_number'];
...
// 			for ($i = 1; $i<=$num; $i++){
// 				$k=$i-1;
// 				${sales.$i._money} = round($fencheng * $affiliate['item'][$k]['level_money'], 2);
// 				${sales.$i._money}=${sales.$i._money}/100;
// 				$total['num'][sales.$i._money]+=${sales.$i._money};
// 				//$logger->warn('金额：'.$total['fencheng'].sales.$i._money.'分成：'.$total['num'][sales.$i._money]);
// 			}

		/*青山老农新增加入订单商品分成金额*/
        $total['market_price'] += $val['market_price'] * $val['goods_number'];
        //判断商品是否不参与分销分成
//        $logger->debug('goods_id = ' . $val['goods_id']);
        if(in_array($val['goods_id'], $commission_unable_arr)){
        	$commission_unable_amount += $val['goods_price'] * $val['goods_number'];
//        	$logger->debug('$commission_unable_amount = ' . $commission_unable_amount);
        }
    }
    ...
    //用订单应付金额核定分成和赠送的积分和红包
    //$fencheng = $total['amount'] - $commission_unable_amount;
    $fencheng = $total['amount'];
//    $logger->debug('$fencheng = ' . $fencheng);
    $total['fencheng'] = $fencheng > 0 ? $fencheng : 0;
//    $logger = LoggerManager::getLogger('lib_order.php');
    $affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
    $num =  count($affiliate['item']);
    for ($i = 1; $i<=$num; $i++){
...
//         $total['will_get_integral'] = $group_buy['gift_integral'];
        //获取应付金额等额的积分 by liuzhy 20160301
        if($GLOBALS['_CFG']['integral_giving_setting'] == 1){
        	$total['will_get_integral'] = ceil($total['amount']);
        }else{
        	$total['will_get_integral'] = $group_buy['gift_integral'];
        }
    }
    ...
    $total['pay_amount'] = $total['goods_price'] - $total['discount'] + $total['tax']	//应付总金额，用于计算可用积分支付额度
    		+ $total['pack_fee'] + $total['card_fee'] - $total['bonus'] - $total['bonus_kill']
    		+ $total['shipping_fee'] + $total['shipping_insure']  - $total['surplus'] ;
    $user_pay_points = 0;
    $integral_scale = 0;
    if ($_SESSION['user_id'] > 0){
    	$user_pay_points = $GLOBALS['db']->getOne("select pay_points from " . $GLOBALS['ecs']->table('users') .
    			" where user_id='" . $_SESSION['user_id'] . "'");
    	$integral_scale = $GLOBALS['db']->getOne("select value from ecs_touch_shop_config where code='integral_scale'");
    	$integral_scale = floatval($integral_scale/100);
    	$integral_scale = round($integral_scale, 2);
    }
    $total['integral_scale'] = $integral_scale;
    $total['order_max_integral'] = ceil(floatval($total['pay_amount'] / 2) / $integral_scale);
    if($total['order_max_integral'] > $user_pay_points){
    	$total['order_max_integral'] = $user_pay_points;
    }
 ...
 * 取得用户当前可用红包
 * @param   int     $user_id        用户id
 * @param   string  $goods_id       商品id串
 * @param   float   $goods_amount   订单商品金额
 * @return  array   红包数组
 */
function user_bonus1($user_id,$goods_id_list , $goods_amount = 0)
{
	$day    = getdate();
	$today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	...
	$sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id " .
			...
			"AND b.user_id = '$user_id' " .
			"AND b.order_id = 0 ".
			"AND t.type_goods_id in ($goods_id_list) ";
	return $GLOBALS['db']->getAll($sql);
}
...
//	$logger = LoggerManager::getLogger('lib_order.php');
    ...
    /* 循环、统计 */
    $sql = "SELECT c.*, IF(c.parent_id, c.parent_id, c.goods_id) AS pid , c.goods_number, g.suppliers_id, s.suppliers_name, g.goods_thumb " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " c" .
            " left join  ecs_goods g on c.goods_id=g.goods_id" .
            " left join ecs_suppliers s on g.suppliers_id = s.suppliers_id" .
...
//	$logger->debug('$goods_list = ' . json_encode($goods_list));
...
function integral_to_give($order)
{
	if($GLOBALS['_CFG']['integral_giving_setting'] == 1){
		//改为实付金额数量的积分赠送
		$od = $GLOBALS['db']->getRow("select * from " . $GLOBALS['ecs']->table('order_info') . " where order_id = '$order[order_id]'");
		return array(
				'custom_points' => $od['money_paid'],
				'rank_points' => $od['money_paid']
		);
	}else{
	    /* 判断是否团购 */
	     if ($order['extension_code'] == 'group_buy')
...
{ 
    ...
    $sqlbonus = "select * from " . $GLOBALS['ecs']->table('user_bonus') . " where  user_id = '$user[user_id]' and bonus_order= '$order_id'";
    
    $userbonus = $GLOBALS['db']->getRow($sqlbonus);
    if(!$userbonus){
	    /* 取得订单应该发放的红包 */
	    ...
	    if ($bonus_list)
	    {
	        ...
	        foreach ($bonus_list AS $bonus)
	        {
	            ...
	            /* 修改用户红包 */
	            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . " (bonus_type_id, user_id ,bonus_order) " .
...
//获取下线发放红包信息
/**
 *$type  3.线下红包    4.绑定手机发红包
 *
 */
function mobile_bonus($type = 3){
	$day    = getdate();
	$today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	$sql = "select t.type_id, t.type_money,t.type_name,u.bonus_id from ecs_bonus_type t ".
			" left join ecs_user_bonus u on t.type_id = u.bonus_type_id " .
			" where t.send_type =  ".$type.
			" AND u.user_id = 0  ".
			" AND t.send_start_date <= '$today' " .
            " AND t.send_end_date >= '$today' " .
            " LIMIT 1";
