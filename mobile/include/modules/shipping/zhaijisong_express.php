...
$shipping_lang = ROOT_PATH.'lang/' .$GLOBALS['_CFG']['lang']. '/shipping/zhaijisong_express.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 配送方式的描述 */
    $modules[$i]['desc']    = 'zhaijisong_express_desc';

    /* 配送方式是否支持货到付款 */
    $modules[$i]['cod']     = TRUE;

    /* 插件的作者 */
    $modules[$i]['author']  = '青山老农电子商务';
    ...
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/dly_zhaijisong_express.jpg';
...
class zhaijisong_express
{
     ...
     * @return null
     */
    function zhaijisong_express($cfg=array())
    {
    ...
    function kuaidi100($invoice_sn){
    	...
    	$url = "http://api.kuaidi.com/openapi.html?id=$kuaidi_key&com=zhaijisong&nu=$invoice_sn&show=0&muti=0&temp=" . time();
//         $url = 'http://m.kuaidi100.com/query?type=zhaijisong&postid=' .$invoice_sn. '&temp='.time();
        return $url;
    }
}
