...
$sql="select order_id from ".$ecs->table('order_info')." where shouhuo_time < ($time-$out_time) and shipping_status=2 and is_separate=0 and (order_status=1 or order_status=5)";
	   ...
	   $tianxin100q=$row['fencheng']+1;
        ...
        if(empty($separate_by))
        {
            ...
            for ($i=0; $i < $num; $i++)
            {
                ...
                {
                    /*t青山老农 新增 Mr.lu  如果上级已经不是分销商，就不分成*/
					//echo $setpoint;
					$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
					$level_register_up = (float)$affiliate['config']['level_register_up'];
