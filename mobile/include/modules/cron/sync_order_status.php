...
$cron_lang = ROOT_PATH . 'lang/' . $GLOBALS ['_CFG'] ['lang'] . '/cron/sync_order_status.php';
...
$logger->info(json_encode($connectionInfo));
$conn == false;
try {
	$conn = sqlsrv_connect ( $cron ['server_ip'], $connectionInfo );
	$logger->debug($conn);
} catch (Exception $e) {
	$logger->error("sqlserver数据库连接异常！IP=" . $cron ['server_ip']);
	$logger->error($e->getMessage());
	$logger->error(json_encode($connectionInfo));
}
...
$cron['dayOfOrderTo'] = intval(0-$cron['dayOfOrderTo']) + 1;
...
// 		if($row['shop_id'] == 'OTHER_13'){
		$order_cond = '';
		if(strlen($row['order_id']) == 13){
			$order_cond = "o.order_sn='".$row['order_id']."'";
		}else{
			$order_cond = "o.order_id='".$row['order_id']."'";
		}
		if(empty($row['logistics_id']) || empty($row['logistics_order'])){
			$sql = "update ecs_order_info o
					set o.shipping_status=1 where " . $order_cond;
		} else {
			$sql = "update ecs_order_info o, ecs_touch_shipping s
					...
					s.shipping_id = '".$shop_mapping[$row['logistics_id'].""]."' and " . $order_cond;
		}

// 		$logger->debug("更新自有商城订单状态: " . $sql);
		$GLOBALS['db']->query($sql);
		$GLOBALS['db']->query("update ecs_order_info set shipping_time=$time where shipping_time=0 and " . $order_cond);
// 		}
