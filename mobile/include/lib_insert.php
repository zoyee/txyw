...
function insert_query_info()
{
    ...
    /* 加入触发cron代码 */
    $cron_method = '<img src="api/cron.php?t=' . gmtime() . '" alt="" style="width:0px;height:0px;" />';
    return  $cron_method;
}
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                    'p.ad_height, p.position_style, a.sort_order ' .
                ...
                'ORDER BY a.sort_order LIMIT ' . $arr['num'];
...
function insert_comments1($arr)
{
	...
	$logger = LoggerManager::getLogger('lib_inser.php');
	$logger->debug("评论:insert_comments1:zmh"); */
	$need_cache = $GLOBALS['smarty']->caching;
	$need_compile = $GLOBALS['smarty']->force_compile;
	...
	$val = $GLOBALS['smarty']->fetch('library/comments_new.lbi');
