...
{
// 	$logger = LoggerManager::getLogger('lib_main.php');
// 	$logger->debug("调用更新用户方法update_user_info()");
    if (!$_SESSION['user_id'])
    {
        return false;
    }
...
//             $logger->debug("查询特殊客户等级：" . $sql);
            $tt = $GLOBALS['db']->getOne($sql);
            if(empty($tt))
            {
//             	$logger->debug("纠正用户等级");
...
function get_pager($url, $param, $record_count, $page = 1, $size = 10)
{
    ...
    if ($pager['styleid'] == 0)
    {
        ...
        $pager['page_param']    = $url . $param_url;
        $pager['array']  = array();
        for ($i = 1; $i <= $page_count; $i++)
        {
            $pager['array'][$i] = $i;
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $navlist[$row['type']][] = array(
            ...
            'opennew'   =>  $row['opennew'],
            'url'       =>  $row['url'],
            'pic'       =>  empty($row['pic'])?'images/ico.png':$row['pic'],
            'ctype'     =>  $row['ctype'],
            'cid'       =>  $row['cid'],
            );
    }
...
function scene_encode($arg){
	if(is_string($arg)){
		return $arg;
	} else if(is_array($arg) && sizeof($arg) == 2){
		$bit = 9;
		$num1_len = strlen($arg[0]);
		$zero = '';
		for($i = $num1_len; $i < $bit; $i++){
			$zero .= "0";
		}
		$ret_code = "1" . $zero . $arg[0];

		$bit = 10;
		$num2_len = strlen($arg[1]);
		$zero = '';
		for($i = $num2_len; $i < $bit; $i++){
			$zero .= "0";
		}
		$ret_code = $ret_code . $zero . $arg[1];

		return $ret_code;
	} else if(is_array($arg) && sizeof($arg) == 3){
		$bit = 9;
		$num1_len = strlen($arg[0]);
		$zero = '';
		for($i = $num1_len; $i < $bit; $i++){
		...
		$bit = 10;
		$num3_len = strlen($arg[2]);
		$zero = '';
		for($i = $num3_len; $i < $bit; $i++){
			$zero .= "0";
		}
		$ret_code = $ret_code . $zero . $arg[2];
...
function scene_decode($code){
	if(strlen($code) <= 10){
		...
		$num1 = substr($code, 0, 10);
		$num2 = substr($code, 10, 10);
		$num1 = intval(substr($num1, 1));
		$num2 = intval($num2);
		return array($num1, $num2);
	} else if (strlen($code) == 30){
		$num1 = substr($code, 0, 10);
		$num2 = substr($code, 10, 10);
		$num3 = substr($code, 20, 10);
		$num1 = intval(substr($num1, 1));
		$num2 = intval($num2);
		$num3 = intval($num3);
