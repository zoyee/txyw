 ...
 * 获取商品的特殊标签
 */
function get_goods_special_icons($goods_id){
	$ret_arr = array();
	$special_key = $GLOBALS['_CFG']['valid_special_icon_key'];
	$special_cat_conf = $GLOBALS['_CFG']['special_cat_conf'];

	$goods = $GLOBALS['db']->getRow("select g.cat_id, is_best, is_new, is_hot, is_promote, special_ico, c.prompt_ico from " .
			 $GLOBALS['ecs']->table('goods') . " g left join " . $GLOBALS['ecs']->table('category') .
			" c on g.cat_id = c.cat_id where goods_id='$goods_id'");
	//添加产品特殊标签
	if($goods['special_ico']){
		array_push($ret_arr, $goods['special_ico']);
	}

	//添加特殊分类标签
	foreach ($special_key as $key){
		if($goods['is_' . $key] && $special_cat_conf[$key]['ico']){
			array_push($ret_arr, $special_cat_conf[$key]['ico']);
		}
	}

	//添加产品分类标签
	if($goods['prompt_ico']){
		array_push($ret_arr, $goods['prompt_ico']);
	}

	//添加其他产品分类标签
	$other_cat = $GLOBALS['db']->getAll("select o.cat_id, c.prompt_ico from " . $GLOBALS['ecs']->table('goods_cat') .
			" o left join " . $GLOBALS['ecs']->table('category') .
			" c on o.cat_id = c.cat_id where o.goods_id='$goods_id'");
	foreach ($other_cat as $row){
		if($row['prompt_ico']){
			array_push($ret_arr, $row['prompt_ico']);
		}
	}

	if($special_cat_conf['mult_switch']){
		return $ret_arr;
	}else if(count($ret_arr) > 0){
		return array($ret_arr[0]);
	}else{
		return array();
	}
}
...
{
// 	$logger = LoggerManager::getLogger('lib_goods.php');
...
//         $logger->debug('$goods_data = ' . json_encode($goods_data));
        $time = gmtime();
        $order_type = $GLOBALS['_CFG']['recommend_order'];
//         $logger->debug('recommend_order_type = ' . $order_type);
...
//         $logger->debug('$type_array = ' . json_encode($type_array));
        ...
        $sql = 'SELECT g.goods_id, g.goods_name,g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.sales_volume_base,".
        ...
        foreach ($result AS $idx => $row)
        {
		    ...
		    $goods[$idx]['sales_count']   = intval(selled_count($row['goods_id'])) + intval($row['sales_volume_base']);
		    ...
		    $goods[$idx]['prompt_ico'] = get_goods_special_icons($row['goods_id']);
		    $goods[$idx]['goods_guide'] = $row['goods_guide'];

            if (in_array($row['goods_id'], $type_array['best']))
            {
            	$goods[$idx]['watermark_img'] =  'watermark_best_small';
                $type_goods['best'][] = $goods[$idx];
            }
            if (in_array($row['goods_id'], $type_array['new']))
            {
            	$goods[$idx]['watermark_img'] =  'watermark_new_small';
                $type_goods['new'][] = $goods[$idx];
            }
            if (in_array($row['goods_id'], $type_array['hot']))
            {
            	$goods[$idx]['watermark_img'] =  'watermark_hot_small';
    ...
    $ret = $type_goods[$type];
//     $logger->debug('get_recommend_goods return: ' . json_encode($ret));
    return $ret;
}
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "g.is_best, g.is_new, g.is_hot, g.is_promote, RAND() AS rnd, g.goods_guide " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        /* 处理商品水印图片 */
        /* 处理商品水印图片 */
        $goods[$idx]['watermark_img'] =  'watermark_promote_small';
    }
 ...
 * @return  void
 */
function get_original_goods_info($goods_id)
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('goods_original') . ' AS g ' .
...
function get_goods_gallery($goods_id)
{
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
        ...
        " WHERE goods_id = '$goods_id' order by img_desc,img_id LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
    ...
    foreach($row as $key => $gallery_img)
    {
		...
		if(empty($flag)){
// 			$row[$key]['img_url']="";
			$row[$key]['img_url'] = get_image_path($goods_id, $gallery_img['img_url'], true, 'gallery');
		}else{
...
"g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name, b.brand_id " .
'FROM ' . $GLOBALS ['ecs']->table ( 'goods' ) . ' AS g ' .
