 ...
 * @return      array
 */
function get_regions($type = 0, $parent = '')
{
//     $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['ecs']->table('region') .
//             " WHERE region_type = '$type' AND parent_id = '$parent'";
    $sql = "select region_id, region_name from ".$GLOBALS['ecs']->table('region')." where parent_id='$parent'";
...
清除 PC端的缓存
*/
function clear_tpl_files_pc($is_cache = true, $ext = '')
{
    $dirs = array();
...
*/
function clear_all_files_pc($ext = '')
{
    return clear_tpl_files_pc(false, $ext) + clear_tpl_files_pc(true,  $ext);
}
        ...
        case 'article':
            ...
            else
            {
				...
				if(!empty($userid)){
					...
					$uri = $rewrite ? 'article-' . $aid : 'article.php?id=' . $aid."&u=".$userid;
				}else{
        ...
        case 'exchange_goods':
            ...
            else
            {
				...
				if(!empty($userid)){
					...
					$uri = $rewrite ? 'exchange-id' . $gid : 'exchange.php?id=' . $gid ."&u=".$userid. '&amp;act=view';
				}else{
...
function gb2py($text, $exp = '')
{
	...
	$data = array();
	$tmp = @file(ROOT_PATH . 'include/codetable/gb-pinyin.table');
	if(!$tmp) return '';
	$tmps = count($tmp);
	for($i = 0; $i < $tmps; $i++) {
		$tmp1 = explode("	", $tmp[$i]);
...
function Recordkeyword($word_www_088x_com, $items = 0, $searchengine = 'ecshop')
{
	if(strlen($word_www_088x_com) < 3 || strlen($word_www_088x_com) > 30 || strpos($word_www_088x_com, ' ') !== false) return;
	$sql_www_088x_com = "SELECT * FROM " .$GLOBALS['ecs']->table('keyword'). " WHERE searchengine='ecshop' AND word='$word_www_088x_com'";
	$r = $GLOBALS['db']->getRow($sql_www_088x_com);
	if($r)
	{
		$items = intval($items) ;
        ...
        $sql_www_088x_com = "UPDATE " . $GLOBALS['ecs']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_www_088x_com);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_www_088x_com   = gb2py($word_www_088x_com);
		$sql_www_088x_com = "INSERT INTO " . $GLOBALS['ecs']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_www_088x_com', '$word_www_088x_com', '$letter_www_088x_com', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_www_088x_com);
		$w_id = $GLOBALS['db']->insert_id();
	}
	if (!empty($w_id))
	{
    	$ip_www_088x_com       = real_ip();
    	$area_www_088x_com     = ecs_geoip($ip);
    	$sql_www_088x_com = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_www_088x_com', '$area_www_088x_com')";
    	$GLOBALS['db']->query($sql_www_088x_com);
	}
}
...
/* wei2 增加 end by www.we10.cn */
/* 代码增加_start   By www.de11.cn */
...
function url_redirect($url,$time=0,$msg='')
{
	//多行URL地址支持
