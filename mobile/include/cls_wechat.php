...
class Wechat{
	...
	public function check_device(){
		if(!isset($_SESSION['device_os'])){
			...
			$_SESSION['device_os'] = $device;
			if(empty($_SESSION['user_id'])){
				$code = $this->get_oauth2_code();
			  	$wxid = $this->get_oauth2_openid($code);

			    
// 			    $wxid=$this->get_douchat_openid();
			    
			  	$GLOBALS['db']->query("update wxch_user set device=$device where wxid='$wxid'");
			}else{
				$user_id = $_SESSION['user_id'];
				$GLOBALS['db']->query("update wxch_user w, ".$GLOBALS['ecs']->table('users')." u set w.device=$device where w.wxid=u.wxid and u.user_id='$user_id'");
			}
		}
	}
  ...
  {
// 	$logger = LoggerManager::getLogger('Wechat.php');
...
// 	   		$logger->debug('$_SESSION[wechat_id] = ' . $_SESSION['wechat_id']);
// 	   		$logger->debug('$_REQUEST[wechat_id] = ' . $_REQUEST['wechat_id']);
...
// 		   $logger->debug('$wechatid = ' . $wechatid);
		   if(!empty($wechatid)){
			   ...
			   if(!$userinfo){
				   ...
				   $_SESSION['wechat_id'] = $wechatid;// added by PRINCE 120029121
			   }else{
...
//取得openid
  ...
  function create_wechat_user($wechatid,$u){
		...
		if (empty($ret)) {
			if (!empty($wechatid)) {
				$GLOBALS['db'] -> query("INSERT INTO `wxch_user` (`subscribe`, `wxid` , `dateline`,`affiliate`,`from_type`) VALUES ('0','$wechatid','$time','$u', 2)");
			}
		} else {
