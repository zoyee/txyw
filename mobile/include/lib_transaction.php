...
function edit_profile($profile)
{
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'wxh');
 ...
 * @return  boolen      $bool
 */
function edit_profile_new($profile)
{
	if (empty($profile['user_id']))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
	...
	$cfg = array();
	$cfg['username'] = $profile['username'];//$GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id='" . $profile['user_id'] . "'");
	if (isset($profile['sex']))
	{
		$cfg['gender'] = intval($profile['sex']);
	}
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, wxid, wxh ".
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_id,invoice_no, shipping_status, pay_status, add_time, " .
    ...
    {
        if ($row['order_status'] == OS_UNCONFIRMED || $row['order_status'] == OS_CONFIRMED || $row['order_status'] == OS_CANCELED)
        {
            //$row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
//        	$row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. 
//        	"\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\"><button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
//        			.$GLOBALS['_LANG']['cancel']."</button></a>";
        	$row['handler'] = '';
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            if ($row['shipping_status'] == SS_SHIPPED)
            {
                ...
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id'].
                "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\"><button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
                		.$GLOBALS['_LANG']['received']."</button></a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                //@$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
                @$row['handler'] = "<button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
                		.$GLOBALS['_LANG']['ss_received']."</button>";
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)
                {
                    ...
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id'].
                "\" ><button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
                		.$GLOBALS['_LANG']['pay_money']."</button></a>";
                }
                else
                {
                	...
                	@$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id'].
                	"\" ><button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
                			.$GLOBALS['_LANG']['view_order']."</button></a>";
                }
        ...
        else
        {
            ...
            //$row['handler'] = '<span style="background: #dadada none repeat scroll 0 0;color:#e60012">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span>';
            $row['handler'] = "<button style=\"width: 82px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">"
            		.$GLOBALS['_LANG']['os'][$row['order_status']] ."</button>";
        }
...
function affirm_received($order_id, $user_id = 0)
{
    ...
    else
    {
		/*青山老农  新增添加收货时间*/
		$time=gmtime();
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "',shouhuo_time='".$time."' WHERE order_id = '$order_id'";
		/*青山老农  新增添加收货时间*/
        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {
         $shipping_code = $GLOBALS['db']->GetOne("SELECT shipping_code FROM ".$GLOBALS['ecs']->table('touch_shipping') ." WHERE shipping_id = '$order[shipping_id]'");
...
function get_user_bouns_list($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (empty($row['order_id']))
        {
            ...
            $row['type_money_new'] = str_replace(".00", "", $row['type_money'].'');
            $row['min_goods_amount_new'] = str_replace(".00", "", $row['min_goods_amount'].'');
        }
        else
        {
