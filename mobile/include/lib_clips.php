 ...
 */
function get_online_payment_list($include_balance = true,$weixin)
{
    if($weixin){
    	$sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc ' .
    			'FROM ' . $GLOBALS['ecs']->table('touch_payment') .
    			" WHERE enabled = 1 AND is_cod <> 1 AND pay_code = 'wx_new_jspay'";
    }else{
    	$sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc ' .
    			'FROM ' . $GLOBALS['ecs']->table('touch_payment') .
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points,rank_points, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
