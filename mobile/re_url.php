...
$logger = LoggerManager::getLogger('re_url.php');
/* 支付方式代码 */
...
function  record_point($keyword,$fromUsername){
		...
		$db=$GLOBALS['db'];
		$lasttime = time();
		$lastdate = date("y-m-d 00:00:00",$lasttime);
...
('$fromUsername', '$keyword' , 1, $lasttime, $dateline);";
			...
			$logger = LoggerManager::getLogger('re_url.php');
			$logger->debug("zmh时间".date("y-m-d H:i:s",$lasttime).':'.date("y-m-d  H:i:s",$db_lasttime));
			$db_lastdate = date("y-m-d 00:00:00",$db_lasttime);
			if(strtotime($lastdate)- strtotime($db_lastdate) > 0)
			{
