...
$logger = LoggerManager::getLogger('flow.php');
...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    $json  = new JSON;
if(!empty($_GET['goods'])){
	$_POST['goods'] = $_GET['goods'];
}elseif(empty($_POST['goods'])){
	$result['error'] = 1;
    ...
    /* 商品限购插件 begin */
    $time_xg_now = gmtime();
    $row_xg = $GLOBALS['db']->getRow("select buymax, buymax_start_date, buymax_end_date from ". $GLOBALS['ecs']->table('goods') ." where goods_id='".$goods->goods_id."'");
    if ($row_xg['buymax'] > 0 && $row_xg['buymax_start_date'] <= $time_xg_now && $row_xg['buymax_end_date'] + 86400 > $time_xg_now)
    {
    	if (!$_SESSION['user_id'])
    	{
    		$result['error'] = 999;
    	...
    	{
    		$result['error'] = 888;
    		$result['message'] = "此商品为限购商品，限购期间每人最多只能订购 ". $row_xg['buymax'] . " 件！";
    		die($json->encode($result));
    	}
    	else
    	{
    		$num_cart_old_1 = $GLOBALS['db']->getOne("select sum(goods_number) from ". $GLOBALS['ecs']->table('cart') ." where session_id = '" . SESS_ID . "' and goods_id= " . $goods->goods_id);
    		$num_cart_old_2 = $GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_status<>0 and o.order_status<>2 and o.order_status<>3 and o.order_id = og.order_id and add_time >= ". $row_xg['buymax_start_date'] ." and add_time < ". ($row_xg['buymax_end_date']+86400) ."  and og.goods_id = " . $goods->goods_id);
    		$num_cart_old_3 = $GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_status=0 and o.order_id = og.order_id and add_time >= ". $row_xg['buymax_start_date'] ." and add_time < ". ($row_xg['buymax_end_date']+86400) ."  and og.goods_id = " . $goods->goods_id);
    		$num_cart_old = $num_cart_old_1 + $num_cart_old_2 + $num_cart_old_3;
    		$num_total = $num_cart_old + intval($goods->number);
    		if ($num_total > intval($row_xg['buymax']))
    		{
    			$result['error'] = 888;
    			$num_else = intval($row_xg['buymax'])-$num_cart_old;
    			$result['message'] = "注意：<br/>此商品促销期间每人限购 ". $row_xg['buymax'] . " 件<br/>";
    			if ($num_cart_old_2)
    			{
    				$result['message'] .="您之前已经成功购买过 $num_cart_old_2 件！<br/>";
    			}
    			if ($num_cart_old_3)
        		{
        			$result['message'] .="您的未付款订单中有 $num_cart_old_3 件！<br/>";
        		}
        		if ($num_cart_old_1)
        		{
        			$result['message'] .="您的购物车中已经存在 $num_cart_old_1 件！<br/>";
        		}
        		if ($num_cart_old_3)
        		{
        			$result['message'] .="请先取消未付款订单再购买！<br/>";
        		} else if($num_else){
...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
    ...
    if (isset($_POST['rec_id']) &&isset($_POST['goods_number']))
    {
        ...
        if ($row_xg['buymax'] > 0 && $row_xg['buymax_start_date'] <= $time_xg_now && $row_xg['buymax_end_date'] + 86400 > $time_xg_now)
        {
        	...
        	elseif ($val > intval($row_xg['buymax']))
        	{
        		$result['error'] = 888;
        		$result['goods_number'] = $row_xg['buymax'];
        		$result['message'] = "此商品为限购商品，限购期间每人最多只能订购 ". $row_xg['buymax'] . " 件！";
        		die($json->encode($result));
        	}
        	else
        	{
        		...
        		$num_cart_old_2 = $GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_status<>0 and o.order_status<>2 and o.order_status<>3 and o.order_id = og.order_id and add_time >= ". $row_xg['buymax_start_date'] ." and add_time < ". ($row_xg['buymax_end_date']+86400) ."  and og.goods_id = " . $goods['goods_id']);
        		$num_cart_old_3 = $GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_status=0 and o.order_id = og.order_id and add_time >= ". $row_xg['buymax_start_date'] ." and add_time < ". ($row_xg['buymax_end_date']+86400) ."  and og.goods_id = " . $goods['goods_id']);
        		$num_cart_old = $num_cart_old_2 + $num_cart_old_3;
        		$num_total = $num_cart_old + $val;
        		if ($num_total > intval($row_xg['buymax']))
        		{
        			$result['error'] = 888;
        			$num_else = intval($row_xg['buymax'])-$num_cart_old;
        ...
        foreach ($cart_goods['goods_list'] as $k=>$goods )
        {
            if ($goods['rec_id'] == $key)
            {
...
{
	//跳转关注二维码页面 liuzhy
	//判断购物车中是否有新人福利的商品，如果有，必须扫码关注成为会员之后才能购买
	$sql = "select distinct cart.goods_id from " . $ecs->table('cart') . " cart, " . $ecs->table('goods_cat') . " gct, (select cat_id from " . $ecs->table('category') . " where login_limit = 1) cg, " . $ecs->table('goods') . " g
			where (cart.goods_id = g.goods_id and g.cat_id = cg.cat_id and cart.session_id='" . SESS_ID . "') 
			or (cart.goods_id = gct.goods_id and gct.cat_id = cg.cat_id and cart.session_id='" . SESS_ID . "')";
	$logger->debug('login_limit_goods check: ' . $sql);		
	$login_limit_goods = $db->getOne($sql);
	$logger->debug('$login_limit_goods = ' . $login_limit_goods);		
	if($login_limit_goods){
		ecs_header("Location: subscribe.php\n");
	}else{
		ecs_header("Location: subscribe.php?buy_flag=1\n");
	}
    exit;
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $smarty->assign('province_list', get_regions( 1 , '86' ));
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($db->getOne($sql) == 0)
    {
        show_message($_LANG['no_goods_in_cart'], '返回继续购物', 'index.php', 'warning');
    }
    ...
    foreach ($cart_goods as $value)
    {
    	...
    	$goods_id_list = $goods_id_list.",".$value["goods_id"];
    }
...
//         $smarty->assign('order_max_integral', $total['order_max_integral']);
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
    	...
    	$user_bonus = user_bonus1($_SESSION['user_id'],$goods_id_list, $total['goods_price']);
        if (!empty($user_bonus))
        {
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    $user_info = user_info($_SESSION['user_id']);
    $integral = intval($_REQUEST['integral'] == '' ? 0 : $_REQUEST['integral']);
    ...
    else
    {
        ...
        else
        {
            ...
            if ($integral > $total['order_max_integral']) {
            	$result['error'] = sprintf($_LANG['integral_too_much'], $total['order_max_integral']);
            }else{
...
//     $flow_points = $total['order_max_integral'];
    $user_points = $user_info['pay_points']; // 用户的积分总数

	if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type)) {
        $result['error'] = $_LANG['no_goods_in_cart'];
    } elseif ($points > $user_points) {
...
{$logger->debug("购物车结算");
   ...
   /* 判断是不是实体商品 */
    ...
    for ($i = 1; $i<=$num; $i++){
        $k=$i-1;
		$tianxin="sales".$i."_money";
        $order[sales.$i._money]=$total['num'][$tianxin];
    }
    
    //设置订单所属的供应商，如果不只一个供应商，需要拆单
    $sql = "select distinct g.suppliers_id from " . $ecs->table('cart') . " c , " . 
    		$ecs->table('goods') . " g where c.goods_id=g.goods_id and c.session_id = '" . SESS_ID . "'";
//    $logger->debug('$sql = ' . $sql);	
    $suppliers = $db->getAll($sql);
   	$order['suppliers_id'] = $suppliers[0]['suppliers_id'];
//   	$logger->debug('suppliers_id = ' . json_encode($suppliers));
	...
	/* 拼团 */
    if ($order['extension_code']=='pintuan')
    {
	    $now_time=$now?$now:gmtime();

	   if($_SESSION['pintuan_pt_id']==0){
			$pintuan = array(
				'user_id'    => $_SESSION['user_id'],
				'user_nickname'  => $_SESSION['pintuan_nickname'],
				'user_head'  => $_SESSION['pintuan_headimgurl'],
				'act_id'  => $_SESSION['extension_id'],
				'act_type'  => GAT_PINTUAN,
				'need_people' => $_SESSION['pintuan_level'],
				'available_people' => $_SESSION['pintuan_level']-1,
				'price' => $_SESSION['pintuan_price'],
				'create_time'  => $now_time,
				'end_time'  => $now_time+$_SESSION['pintuan_time_limit']*60*60
			);
			$db->autoExecute($ecs->table('pintuan'), $pintuan, 'INSERT');
    		$new_pt_id = $db->insert_id();
		}

		$pintuan_orders = array(
				'pt_id'  => $_SESSION['pintuan_pt_id']?$_SESSION['pintuan_pt_id']:$new_pt_id,
				'act_id'  => $_SESSION['extension_id'],
				'act_user'  => $_SESSION['pintuan_act_user'],
				'follow_user'    => $_SESSION['user_id'],
				'follow_user_nickname'  => $_SESSION['pintuan_nickname'],
				'follow_user_head'  => $_SESSION['pintuan_headimgurl'],
				'order_id' => $new_order_id ,
				'follow_time'  => $now_time
			);
		$db->autoExecute($ecs->table('pintuan_orders'), $pintuan_orders, 'INSERT');
            ...
            if (virtual_goods_ship($virtual_goods,$msg, $order['order_sn'], true))
            {
                ...
                if ($db->getOne($sql) <= 0)
                {
                    ...
                    if ($order['user_id'] > 0)
                    {
                        ...
                        $logger = LoggerManager::getLogger('flow.php');
                        $logger->debug('zmh-OK:'.$integral['rank_points'].":".$integral['rank_points']);
    ...
    //增加销量统计用于排序 by wang start 修改：改为付款后增加销量
...
//     $db->query($sql);
	...
	if($payment['pay_code'] == 'wx_new_jspay') {
		...
		if($order['order_amount'] == 0 && $autoload=="yes"){
			include ('wxch_order.php');
		}
		$sql ="select autoload from `wxch_order` where order_name='remind'";
		$autoload=$db->getOne($sql);
		if($autoload=="yes"){
			include ('wxch_order_remind.php');
		}
...
elseif($_REQUEST['step'] == 'ok')
{
    ...
    send_order_bonus($order['order_id']);
    //变更商品销量
    $sql = "update " . $GLOBALS['ecs']->table('goods') . " gg,
			(select g.goods_id, g.goods_number as count from " . $GLOBALS['ecs']->table('order_info') . " o , " . $GLOBALS['ecs']->table('order_goods') . " g
			where o.order_id = g.order_id and o.order_id='" . $order['order_id'] . "' and o.order_status >0) t
			 set gg.sales_count = gg.sales_count + t.count where gg.goods_id = t.goods_id";
    $db->query($sql);
...
elseif ($_REQUEST['step'] == 'update_cart')
{
	...
	{
		$sql = "select g.goods_id, g.goods_name,  g.buymax, g.buymax_start_date, g.buymax_end_date from ".$ecs->table('cart'). " AS c left join ".$ecs->table('goods'). " AS g on c.goods_id=g.goods_id  where c.rec_id=".$key;
		$row = $db->getRow($sql);
		if ($row['buymax'] >0 && $row['buymax_start_date'] < $time_xg_now  && $row['buymax_end_date'] > $time_xg_now)
		{
			if ($val > intval($row['buymax']))
			{
				show_message("此商品为限购商品，限购期间每人最多只能订购". $row['buymax'] . " 件！", $_LANG['back_to_cart'], 'flow.php');
				exit;
			}
			else
			{
				...
				if ($num_total > intval($row['buymax']))
				{
					...
					if ($num_cart_old)
					{
						$message .="您以前已经成功购买过 <font color=#330099>$num_cart_old</font> 件！<br>所以，";
					}
...
else
{
    ...
    /* 取得商品列表，计算合计 */
    $cart_goods = get_cart_goods();
    $logger->debug('$cart_goods = ' . json_encode($cart_goods));
    //修改---商品按供应商分类
	$tmp_suppliers=array();
	$suppliers=array();
	foreach ($cart_goods['goods_list'] as $v){
	    if($v['suppliers_id'] > 0){
	        $suppliers[$v['suppliers_id']] = $v['suppliers_name'];
	        $tmp_suppliers[$v['suppliers_id']][] = $v;
	    }else{
	        $suppliers['qita'] = $GLOBALS['_CFG'].shop_name;
	        $tmp_suppliers['qita'][] = $v;
	    }
	}
    
    $smarty->assign('brand',$suppliers);
    $smarty->assign('goods_list_brand', $tmp_suppliers);
  ...
  /*  $html = "<tr>";
   //echo var_dump($favourable_list);
		    ...
		    foreach ($favourable['gift'] as $gift)
		    {
		    	...
		    	if($gift['num'] % 2)
		    	{
		    		$html = $html."<td style='text-align:center'>
		        		<a href='goods.php?id=".$gift['id']."' target='_blank' ><img class='lazy' border='0' src='".$gift['img']."' style='height: 9rem; width: 9rem;' title='".$gift['name']."'></a><br/>
		        		<a href='goods.php?id=".$gift['id']."' target='_blank' title='".$gift['name']."' style='padding: 0 0rem;font-size: 12px;' class='f6'><div style='text-overflow: ellipsis; width: 149px;overflow: hidden;white-space: nowrap;'>".$gift['name']."</div></a><br/>
		        		<span style='color:red; margin-right: 90px;'><b>+".$gift['price']."元</b></span> <input type='checkbox' value='".$gift['id']."' name='gift[]' />
		        		</td>";
		    	}else{
		    		$html = $html."<td style='text-align:center'>
		        		<a href='goods.php?id=".$gift['id']."' target='_blank' ><img class='lazy' border='0' src='".$gift['img']."' style='height: 9rem; width: 9rem;' title='".$gift['name']."'></a><br/>
    ...
    /* 增加是否在购物车里显示商品图 */
    $logger->debug('show_goods_thumb = ' . $GLOBALS['_CFG']['show_goods_in_cart']);
    ...
    {
    	if($gl_num == 0){
    		$gl_goods_id = $value["goods_id"];
    	}
    	$gl_num++;
    }
    $linked_goods = get_linked_goods($gl_goods_id);
...
function flow_available_points()
{
	...
	$order = flow_order_info();
	$total = order_fee($order, $cart_goods, $consignee);
	return $total['order_max_integral'];
...
function favourable_list($user_rank)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach ($favourable['gift'] as $key => $value)
        {
        	...
        	$favourable['gift'][$key]['img'] = "/$img";
        	$favourable['gift'][$key]['num'] = $num;
