...
$logger = LoggerManager::getLogger('cron.php');
//require('../include/lib_time.php');
$timestamp = gmtime();
...
{
    if (file_exists(ROOT_PATH . 'include/modules/cron/' . $cron_val['cron_code'] . '.php'))
    {
    	...
    	if($timestamp > $cron_val['nexttime']){
    		$logger->debug("执行计划任务" . $cron_val['cron_name'] . "(".$cron_val['cron_code'].")");
//     		$logger->debug("时间" . $timestamp);
        	include_once(ROOT_PATH . 'include/modules/cron/' . $cron_val['cron_code'] . '.php');
        	$close = $cron_val['run_once'] ? 0 : 1;
        	$next_time = get_next_time($cron_val['cron']);
//         	$logger->debug("下次执行时间" . $next_time);
    ...
    {
        $error_log[] = make_error_arr('include/modules/cron/' . $cron_val['cron_code'] . '.php not found!',__FILE__);
    }
...
{
    $y  = local_date('Y', $GLOBALS['timestamp']);	//年 2016
    $mo = local_date('n', $GLOBALS['timestamp']);	//月 1-12
    $d  = local_date('j', $GLOBALS['timestamp']);	//日 1-31
    $w  = local_date('w', $GLOBALS['timestamp']);	//星期 0-6
    $h  = local_date('G', $GLOBALS['timestamp']); 	//小时 0-23
    $sh = $sm = 0;
    $sy = $y;
    if ($cron['day'])
    {
...
function get_cron_info()
{
    ...
    $sql   = "SELECT * FROM " . $GLOBALS['ecs']->table('crons') . " WHERE enable = 1 AND nextime < $GLOBALS[timestamp] order by nextime";
