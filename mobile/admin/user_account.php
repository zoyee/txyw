...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    //var_dump('user_account_list.htm');
...
//-- 设置提现额度 甜心 1 0  0 新增
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'tixian_setting')
{
	$sql = "SELECT limit_account_replay FROM wxch_config ";
	$limit_account_replay = $db->getOne($sql);
	$limit_account_replay = intval($limit_account_replay);
	$smarty->assign('limit_account_replay', $limit_account_replay);
	$smarty->display('tixian_setting.htm');
}
if ($_REQUEST['act'] == 'tixian_query')
{
	$limit_account_replay = intval($_POST['limit_account_replay']);
	$sql = "update wxch_config set limit_account_replay = ".$limit_account_replay."  ";
	$res = $db->query($sql);
	$smarty->assign('limit_account_replay', $limit_account_replay);
	$smarty->display('tixian_setting.htm');
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($_REQUEST['act'] == 'edit')
    {
		...
		/* 取得会员微信ID */
        $sql = "SELECT wxid FROM " .$ecs->table('users'). " WHERE user_id = '$user_account[user_id]'";
        $wxid = $db->getOne($sql);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    $user_name = $db->getOne($sql);
/* 取得会员微信ID */
        $sql = "SELECT wxid FROM " .$ecs->table('users'). " WHERE user_id = '$account[user_id]'";
        $wxid = $db->getOne($sql);
    /* 模板赋值 */
...
elseif ($_REQUEST['act'] == 'action')
{
	...
	//var_dump($amount);
        	...
        	if($sendhb['result_code']!='SUCCESS'){
				sys_msg($sendhb['return_msg'], 0, $link);
				exit;
			}
        ...
        elseif ($is_paid == '0')
        {
            ...
            $db->query($sql);
        }elseif ($is_paid == '2'&&$account['process_type'] == '0'){					
			$amount=0;
			update_user_account($id, $amount, $admin_note, $is_paid);		
		}
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        {
            $where .= "AND ua.paid_time >= " . $filter['start_date']. " AND ua.paid_time < '" . $filter['end_date'] . "'";
        }
