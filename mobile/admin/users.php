...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id,u.user_name, u2.user_name as parent_username, u.qq, u.msn,
        u.office_phone, u.home_phone, u.mobile_phone,u.password_account_replay,u.limit_account_replay, u.suppliers_id".
    ...
    if ($row)
    {
		...
		$user['suppliers_id']   = $row['suppliers_id'];
    }
    else
    {
          $link[] = array('text' => $_LANG['go_back'], 'href'=>'users.php?act=list');
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $rank = empty($_POST['user_rank']) ? 0 : intval($_POST['user_rank']);
    $credit_line = empty($_POST['credit_line']) ? 0 : floatval($_POST['credit_line']);
    $password_account_replay = empty($_POST['password_account_replay']) ? '' : MD5(trim($_POST['password_account_replay']));
	$limit_account_replay=empty($_POST['limit_account_replay']) ? '' :trim($_POST['limit_account_replay']);
    $users  =& init_users();

    if (!$users->edit_user(array('username'=>$username, 'password'=>$password, 'email'=>$email, 'gender'=>$sex, 'bday'=>$birthday ,'password_account_replay'=>$password_account_replay ), 1))
    {
    	$logger->debug('$users->error = ' . $users->error);
    ...
    if(!empty($password))
    {
			...
			$db->query($sql);
			$sql1="UPDATE ".$ecs->table('users'). "SET `password_tianxin`='".$password."' WHERE user_name= '".$username."'";
			$db->query($sql1);
	}
    /* 更新用户扩展字段的数据 */
    $sql = 'SELECT id FROM ' . $ecs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
	...
	if(!empty($password_account_replay)){
		$db -> query("update ".$ecs->table('users')." set password_account_replay = '$password_account_replay' where user_id = '$user_id' ");
		}
	if(!empty($limit_account_replay)){

		$db -> query("update ".$ecs->table('users')." set limit_account_replay = '$limit_account_replay' where user_id = '$user_id' ");
	}
    ...
    $other['credit_line'] = $credit_line;
    $other['user_rank'] = $rank;
    $other['suppliers_id'] = $_REQUEST['suppliers_id'];
	...
	/*甜心 100  开发9级分销兼容 程序Start*/
	if(empty($parent_id)){

		$sql ="UPDATE ". $ecs->table('users') ." SET sales1_id = '',sales2_id = '',sales3_id='',sales4_id='',sales5_id=''    WHERE `user_id` = '$user_id';";
		//开始执行绑定上下级关系
		$db -> query($sql);
		$children = $db->getAll("select user_id from " . $ecs->table('users') . " where parent_id='$user_id'");
		foreach($children as $child){
			$sql ="UPDATE ". $ecs->table('users') ." SET sales2_id = '',sales3_id='',sales4_id='',sales5_id=''    WHERE `user_id` = '$child[user_id]';";
			$db -> query($sql);
		}
		...
		$sales2_sql="SELECT * FROM ". $ecs->table('users') ." WHERE `user_id` = '$parent_id'";
		...
		if(!empty($sales2info)){
			...
			$sql ="UPDATE ". $ecs->table('users') ." SET sales1_id = '$parent_id',sales2_id = '$sales2_id',sales3_id='$sales3_id',sales4_id='$sales4_id',sales5_id='$sales5_id'    WHERE `user_id` = '$user_id';";
			//开始执行绑定上下级关系
			$db -> query($sql);
		}
	}

	$sql ="UPDATE wxch_user SET affiliate = '".$parent_id."'    WHERE `uname` = '".$username."';";
	$db -> query($sql);
...
elseif ($_REQUEST['act'] == 'remove_parent')
{
    ...
    $username = $db->getOne($sql);
	/*青山老农 修复脱离上下级关系*/
	$sql ="UPDATE ". $ecs->table('users') ." SET sales1_id = '',sales2_id = '',sales3_id='',sales4_id='',sales5_id=''    WHERE `user_id` = '".$_GET['id']."';";
	$db -> query($sql);
...
elseif ($_REQUEST['act'] == 'share_list')
{
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
            $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,wxid, mobile_phone ".
                    " FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id IN($up_uid)" .
                    " ORDER by level, user_id";
			...
			foreach($user_info as $key=>$value){
				...
				//$user_info[$key]['reg_time'] = local_date($GLOBALS['_CFG']['date_format'], $value['reg_time']);
			}
	...
	foreach($user_list['user_list'] as $key =>$value)
	{
		...
		if($value['level']==1){
			...
			$value['reg_time'] = local_date($GLOBALS['_CFG']['date_format'], $value['reg_time']);
			$new_arr1[]=$value;
		}
	...
	$res=array($new_arr1,$new_arr2,$new_arr3,$new_arr4,$new_arr5);
	for ($i = 1; $i<=$num; $i++){
		$k=0;
		$k=$i-1;
		$smarty->assign('user_list'.$i,        $res[$k]);
		$menuarr[$i]=$i;
	}
 ...
 * 解除会员提现密码
 */
elseif($_REQUEST['act'] == 'remove_account_pwd'){
	$user_id = $_REQUEST['user_id'];
	$sql = "update " . $GLOBALS['ecs']->table('users') . " set password_account_replay='' where user_id='$user_id'";
	$GLOBALS['db']->query($sql);
	/* 记录管理员操作 */
	admin_log($user_id, 'remove', 'account_pwd');
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
        {
            ...
            $filter['mobile_phone'] = json_str_iconv($filter['mobile_phone']);
        }
        ...
        if($join_wxch_flag){
        	$ex_where .=" AND w.uname<>'' ";
        }
//         $logger = LoggerManager::getLogger('user.php');

        if($join_wxch_flag){
//         	$logger->debug("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . " left join wxch_user w on user_name = w.uname " . $ex_where);
        	$filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . " left join wxch_user w on user_name = w.uname " . $ex_where);
        }else{
//         	$logger->debug("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . $ex_where);
        	$filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . $ex_where);
        }

        /* 分页大小 */
        $filter = page_and_size($filter);
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, mobile_phone ".
                " FROM " . $GLOBALS['ecs']->table('users') ;
        if($join_wxch_flag){
        	$sql .= " left join wxch_user w on user_name = w.uname ";
        }
        $sql .= $ex_where .
...
//     $logger->debug($sql);
    ...
    for ($i=0; $i<$count; $i++) {
        ...
        $uname_str .= $i !=  0 ? ",'" . $user_list[$i]['user_name'] . "'" : "'" . $user_list[$i]['user_name'] . "'";
    }
//     $logger->debug($uname_str);
	if($count > 0){
	    $nicknames = $GLOBALS['db']->getAll("select uname, nickname from wxch_user where uname in(". $uname_str. ")");
	    for ($i=0; $i<$count; $i++) {
	    	for ($j=0, $jc = count($nicknames); $j<$jc; $j++) {
	    		if($user_list[$i]['user_name'] == $nicknames[$j]['uname']){
	    			$user_list[$i]['nickname'] = $nicknames[$j]['nickname'];
	    			break;
	    		}
	    	}
	    }
