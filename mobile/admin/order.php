...
elseif ($_REQUEST['act'] == 'list')
{
	...
	//拼团状态 Start  added by PRINCE
	$orders=$order_list['orders'];
    foreach ($orders as $key => $order_id){
		   if($orders[$key]['extension_code']=='pintuan'){
			    $pintuan = get_pintuan_by_orderid($orders[$key]['order_id']);
				if( empty($pintuan ) || $pintuan['status']==2 ){
					$pintuan['status']='失败';
				}elseif($pintuan['status']==1){
					$pintuan['status']='成功';
				}else{
					$pintuan['status']='进行中';
				}
                $orders[$key]['pintuan_status'] =$pintuan['status'];
                $orders[$key]['pt_id'] =$pintuan['pt_id'];
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $order['shipping_time'] = $order['shipping_time'] > 0 ?
        date($_CFG['time_format'], $order['shipping_time']) : '';
	...
	if($order['extension_code']=='pintuan'){
			    $pintuan = get_pintuan_by_orderid($order['order_id']);
				if( empty($pintuan ) || $pintuan['status']==2 ){
					$pintuan['status']='失败';
				}elseif($pintuan['status']==1){
					$pintuan['status']='成功';
                ...
                $order['pintuan_status'] =$pintuan['status'];
                $order['pt_id'] =$pintuan['pt_id'];
	}
    //拼团状态 Start
    ...
    /* 取得申请退货理由 */
    /*$sql = "SELECT back_reason FROM " . $ecs->table('back_order_reason') . " WHERE order_id = '$order[order_id]'";
    $back_reason = $db->getOne($sql);
    $smarty->assign('back_reason', $back_reason);*/
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    if ('user' == $step)
    {
        ...
        $order = array(
            ...
            'from_ad'           => 0,
            'referer'           => $_LANG['admin'],
            //'postscript'        => '管理员'.$_SESSION[admin_name].'于'.date('Y-m-d H:i:s').'添加订单'
            'to_buyer'        => '管理员'.$_SESSION[admin_name].'于'.date('Y-m-d H:i:s').'添加订单'
        );
...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['print']))
    {
        ...
        {
        	//$html .= '订单号'.$order_sn.'<br>';
        	if (empty($order_sn))
        	{
        		continue;
        	}
            /* 取得订单信息 */
            $order = order_info($order_sn, $order_sn);
            if (empty($order))
            {
                continue;
            }
    ...
    elseif (isset($_POST['export']))
    {
    	...
    	//填入表头主标题
    	$PHPExcel->getActiveSheet()->setCellValue('A1', $_CFG['shop_name'].'订单列表'.$_POST['order_id']);
    	//填入表头副标题
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    if ('refuse' == $operation)
    {
    	...
    	'SELECT ' .
    	"order_id, '$username', '".OS_CONFIRMED."', shipping_status, pay_status, 0, '拒绝退货', '" .gmtime() . "' " .
    	'FROM ' . $GLOBALS['ecs']->table('order_info') . " WHERE order_sn = '".$order['order_sn']."'";
    ...
    elseif ('return' == $operation)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            if($goods_num['goods_number'] == $goods_num['send_number'])
            {
                /* 计算并退回积分 */
                $integral = integral_to_give($order);
                log_account_change($order['user_id'], 0, 0, (-1) * intval($order['money_paid']), (-1) * intval($order['money_paid']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
            }else if($_REQUEST['refund'] == 3){
            	//$integral = integral_to_give($order);
            	log_account_change($order['user_id'], 0, 0, (-1) * intval($order['money_paid']), (-1) * intval($order['money_paid']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
            }
            /* todo 计算并退回红包 */
...
function operable_list($order)
{
    ...
    }elseif(OS_RETURNED_PEDDING == $os)
    {
    	$list['refuse'] = true; // 拒绝退货
    	$list['return'] = true; // 退货
    }
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($filter['to_buyer']){
        	$where .= " AND o.to_buyer LIKE '%" . mysql_like_quote($filter['to_buyer']) . "%'";
        }
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid,o.bonus," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel,o.mobile, o.extension_code, o.extension_id, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, o.pay_time, o.shipping_time,o.province,o.city,o.district,o.shouhuo_time," .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer, u.mobile_phone as buyer_mobile,u.user_id ".
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['formated_bonus'] = price_format($value['bonus']);
        $row[$key]['formated_total_fee'] = price_format($value['total_fee']);
        $row[$key]['short_order_time'] = local_date('m-d H:i:s', $value['add_time']);
        $row[$key]['shouhuo_time'] = local_date('m-d H:i:s', $value['shouhuo_time']);
        $row[$key]['short_pay_time'] = $value['pay_time'] > 0 ? local_date('m-d H:i:s', $value['pay_time']) : '';
        $row[$key]['short_shipping_time'] = $value['shipping_time'] > 0 ? date('m-d H:i:s', $value['shipping_time']) : '';
...
//拼团
function get_pintuan_by_orderid($order_id)
{
	$sql = "SELECT pt.* " .
				" FROM  " . $GLOBALS['ecs']->table('pintuan_orders') . " AS pto  " .
				" LEFT JOIN " . $GLOBALS['ecs']->table('pintuan') . " AS pt ON pt.pt_id  = pto.pt_id  " .
				" WHERE pto.order_id=".$order_id."  ";
