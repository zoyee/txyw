...
require('wxch_lg.php');
$logger = LoggerManager::getLogger('wxch-ent.php');
...
if($_REQUEST['act'] == 'wxconfig')
{
	...
	if(!empty($_POST['token']))
	{
		...
		if($ret)
		{
			sys_msg('青山老农提示您设置成功',0,$link);
		}
		else
		{
			sys_msg('青山老农提示您设置失败',0,$link);
		}
	}
	else
	{
...
elseif($_REQUEST['act'] == 'wx_msg_list'){
	$all_row = $db->getAll("select * from wxch_order order by id desc");
	$smarty->assign('listData', $all_row);
	$smarty->display('wx_msg_list.htm');
}
elseif($_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'add'){
	$id = $_REQUEST['id'];
	if(!empty($id)){
		$row = $db->getRow("select * from wxch_order where id='$id'");
		$smarty->assign('data', $row);
	}

	$smarty->assign('action', $_REQUEST['act']);
	$smarty->display('wx_msg_info.htm');
}elseif($_REQUEST['act'] == 'do_test'){
	$user_name = $_REQUEST['user_name'];
	$order_name = $_REQUEST['order_name'];
	$row = $db->getAll("select wxid from wxch_user where uname='$user_name'");
	if($row && count($row) == 1){
		$wxid = $db->getOne("select wxid from wxch_user where uname='$user_name'");
		$_REQUEST['touser'] = $wxid;
		require_once (ROOT_PATH . 'api/weixin_api.php');
		$weixin_api = new weixin_api();
		$ret = $weixin_api->send_template_msg($order_name, $_REQUEST);
		if($ret->errcode > 0){
			$link[] = array('href' =>'wxch-ent.php?act=wx_msg_list', 'text' => '微信提醒设置');
			sys_msg($ret->errmsg,1,$link);
		}else{
			$link[] = array('href' =>'wxch-ent.php?act=wx_msg_list', 'text' => '微信提醒设置');
			sys_msg('操作成功，请到公众号查看',0,$link);
		}
	}else{
		$link[] = array('href' =>'wxch-ent.php?act=wx_msg_list', 'text' => '微信提醒设置');
		sys_msg('没有找到匹配的用户',1,$link);
	}
...
// 	$logger->debug($image);
	...
	$param = array(
			...
			"autoload" => isset($_REQUEST['autoload']) ? $_REQUEST['autoload'] : 'no',
			"use_template" => isset($_REQUEST['use_template']) ? $_REQUEST['use_template'] : '0',
			"template_id" => $_REQUEST['template_id'],
			"template_content" => $_REQUEST['template_content'],
			"image" => $image
	);
	if(empty($id)){
		$sql = "insert into wxch_order (order_name, title, content, autoload, use_template, template_id, template_content, image)
				values('" . $param['order_name'] . "','" . $param['title'] . "','" . $param['content'] . "',
				'" . $param['autoload'] . "','" . $param['use_template'] . "','" . $param['template_id'] . "',
				'" . $param['template_content'] . "','" . $param['image'] . "')";
	}else{
		$sql = "update wxch_order set order_name='" . $param['order_name']
				. "',title='" . $param['title'] . "',
	...
	else
	{
		...
		foreach ($ret as $k => $v)
		{
			switch ($v['point_name'])
			{
				...
				break;
				case 'share_arpoint':$ret[$k]['name'] = '分享文章页面点击赠送积分';
				break;
			}
		}
		$form_act = 'point';
...
elseif($_REQUEST['act'] == 'share_shouye')
{


	$wxch_lang['ur_here'] = '分享首页缩略图设置';
	...
	if($_POST)
	{
		...
		if($image)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 5;";
		}
		elseif($http_ret1 or $http_ret2)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 5;";
		}
		else
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 5;";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	else
	{
		...
		$ret =$db->getRow($sql);
		$smarty->assign('order',$ret);
		$smarty->assign('form_act','share_shouye');
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->display('wxch_share_shouye.html');
	}
}
elseif($_REQUEST['act'] == 'share_danpin')
{


	$wxch_lang['ur_here'] = '分享单品缩略图设置';
	if($_POST)
	{
		$title = "分享单品缩略图设置";
		$image = wxch_upload_file($_FILES['image']);
		...
		if($image)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 6;";
		}
		elseif($http_ret1 or $http_ret2)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 6;";
		}
		else
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` =6;";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	{
		$sql = "SELECT * FROM `wxch_order` WHERE `id` = 6";
		$ret =$db->getRow($sql);
		$smarty->assign('order',$ret);
		$smarty->assign('form_act','share_danpin');
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->display('wxch_share_danpin.html');
	}
}
/*青山老农开发 客户下单通知管理员*/
elseif($_REQUEST['act'] == 'remind')
{
	$wxch_lang['ur_here'] = '是否开启提醒';
	if($_POST)
	{
		$title = "下单通知管理员";
		$image = wxch_upload_file($_FILES['image']);
		...
		if(!empty($content)){
			...
			if(empty($wxid)){
				sys_msg('管理员会员号不存在请重新设置',0,$link);
			}
		}
		if($image)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload',`content` = '$content' WHERE `id` = 7;";
		}
		elseif($http_ret1 or $http_ret2)
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload',`content` = '$content' WHERE `id` = 7;";
		}
		else
		{
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload',`content` = '$content' WHERE `id` =7;";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	{
		$sql = "SELECT * FROM `wxch_order` WHERE `id` = 7";
		$ret =$db->getRow($sql);
		$smarty->assign('order',$ret);
		$smarty->assign('form_act','remind');
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->display('wxch_remind.html');
	}
}
...
elseif($_REQUEST['act'] == 'hongbao_reply')
{
	//admin_priv('wx_reorder');
    //Add by ZhangNu

	$wxch_lang['ur_here'] = '送红包提醒';
	if($_POST)
	{
		$title = $_POST['title'];
		...
		if($image)
		{
			//$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 8
            $sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload', `content` = '$content' WHERE `id` = 8";
		}
		elseif($http_ret1 or $http_ret2)
		{
		   //$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 8";
        	$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload', `content` = '$content' WHERE `id` = 8";
		}
		else
		{
			//$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 8";
            $sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' , `content` = '$content'WHERE `id` = 8";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	{
		$sql = "SELECT * FROM `wxch_order` WHERE `id` = 8";
		$ret =$db->getRow($sql);
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->assign('order',$ret);
		$smarty->assign('form_act','hongbao_reply');
		$smarty->display('wxch_hongbao_reply.html');
	}
}

elseif($_REQUEST['act'] == 'smgz_reply')
{
	//admin_priv('wx_reorder');
	//Add by zmh

	$wxch_lang['ur_here'] = '海报扫描二维码关注提醒';
	if($_POST)
	{
		$title = $_POST['title'];
		...
		{
			//$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload' WHERE `id` = 8
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$image',`autoload` = '$autoload', `content` = '$content' WHERE `id` = 9";
		}
		elseif($http_ret1 or $http_ret2)
		{
			//$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload' WHERE `id` = 8";
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`image` = '$img_url',`autoload` = '$autoload', `content` = '$content' WHERE `id` = 9";
		}
		else
		{
			//$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' WHERE `id` = 8";
			$sql = "UPDATE `wxch_order` SET `title` = '$title',`autoload` = '$autoload' , `content` = '$content' WHERE `id` = 9";
		}
		$db->query($sql);
		$href = 'wxch-ent.php?act='.$act;
		$link[] = array('href' => $href, 'text' => $wxch_lang['ur_here']);
	...
	{
		$sql = "SELECT * FROM `wxch_order` WHERE `id` = 9";
		$ret =$db->getRow($sql);
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->assign('order',$ret);
		$smarty->assign('form_act','smgz_reply');
		$smarty->display('wxch_smgz_reply.html');
	}
}
