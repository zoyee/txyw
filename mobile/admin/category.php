...
if ($_REQUEST['act'] == 'insert') {
    ...
    $cat['cat_img'] = empty($_POST['cat_img']) ? '' : $_POST['cat_img'];
    $cat['prompt_ico'] = empty($_POST['prompt_ico']) ? '' : $_POST['prompt_ico'];
...
if ($_REQUEST['act'] == 'update') {
    ...
    $cat['style'] = !empty($_POST['style']) ? trim($_POST['style']) : '';
    $cat['grade'] = !empty($_POST['grade']) ? intval($_POST['grade']) : 0;
    $cat['login_limit'] = !empty($_POST['login_limit']) ? intval($_POST['login_limit']) : 0;
    $cat['filter_attr'] = !empty($_POST['filter_attr']) ? implode(',', array_unique(array_diff($_POST['filter_attr'], array(
        0
    )))) : 0;
    $cat['cat_recommend'] = !empty($_POST['cat_recommend']) ? $_POST['cat_recommend'] : array();
    ...
    if(empty($img_name))
    {
    	$cat['cat_ico'] = empty($_POST['hid_cat_img']) ? '' : $_POST['hid_cat_img'];
    }
    //$prompt_img_name = basename($image->upload_image($_FILES['prompt_ico'], 'cat_ico'));
    $cat['prompt_ico'] = $_REQUEST['prompt_ico'];
    $cat['cat_img'] = $_REQUEST['cat_img'];
