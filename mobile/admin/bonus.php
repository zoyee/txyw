...
if ($_REQUEST['act'] == 'add')
{
    ...
    assign_query_info();
    $smarty->display('bonus_type_info1.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $type_goods_id     = !empty($_POST['type_goods_id'])    ? intval($_POST['type_goods_id'])    : 0;
    $type_goods_name    = !empty($_POST['type_goods_name'])    ? $_POST['type_goods_name']    : '';

    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,type_goods_id,type_goods_name)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$ecs->table('bonus_type'). " SET ".
           ...
           "type_goods_id   = '$type_goods_id', ".
           "type_goods_name = '$type_goods_name', " .
...
if ($_REQUEST['act'] == 'send')
{
    ...
    }elseif ($_REQUEST['send_by'] == SEND_BY_MOBILE)
    {
        $smarty->assign('type_list',    get_bonus_type(4));
...
if($_REQUEST['act'] == 'get_goods'){
	...
	$is_on_sale = isset($_REQUEST['is_on_sale'])?$_REQUEST['is_on_sale']:"1";
	$goods_list = array();
	
	$sql = "select goods_id,goods_sn,goods_name,is_on_sale from ecs_goods where  goods_name like '%$goods_name%'";
	if($is_on_sale != ""){
		$sql = $sql. " and is_on_sale = $is_on_sale";
	}
	$sql = $sql. " LIMIT 10";
	$res = $GLOBALS['db']->query($sql);
	...
	$smarty->assign('goods_list', $goods_list);
	$str = $smarty->fetch('bonus_goods.htm');
	$goods[] = array('str' => $str);
