...
if($_REQUEST['act'] == 'add') 
{
	...
	if($_POST) 
	{
		...
		{
			$update_sql = "UPDATE `wxch_prize_append` SET `prize_name` = '$prize_name[$k]',`prize_number` = '$prize_number[$k]',`prize_value` = '$prize_value[$k]' WHERE `id` =$prize_id[$k] AND `prize_id` = '$pid';";
			$db->query($update_sql);
		}
...
elseif($_REQUEST['act'] == 'news') 
{
	...
	if($_POST) 
	{
		...
		{
			$insert_sql = "INSERT INTO `wxch_prize_append` (`prize_id`, `prize_name`, `prize_value`,`prize_number`) VALUES ('$post_pid', '$prize_name[$k]', '$prize_value[$k]','$prize_number[$k]')";
			$db->query($insert_sql);
		}
		$url = 'wxch_prize.php?act=edit&pid='.$pid;
		$link[] = array('href' =>$url, 'text' => $wxch_lang['ur_here']);
...
elseif($_REQUEST['act'] == 'add_prize') 
{
	...
	if($_POST) 
	{
		...
		$time = time();
		$insert_sql = "INSERT INTO `wxch_prize` (`title`, `fun`, `num`, `count`, `loop`, `starttime`, `endtime`, `dateline`, `is_show`, `price`) VALUES
('$name', '$fun', '$num', 0, '$loop', '$starttime', '$endtime', '$time',$is_show,$price);";
		$db->query($insert_sql);
		$pid = $db->insert_id();
...
elseif($_REQUEST['act'] == 'edit_prize') 
{
	...
	if($_POST) 
	{
		...
		if(empty($price)){
			sys_msg('所需购物价钱不能为空',0,$link);
		} */
		$endtime = strtotime($_POST['endtime']);
		$time = time();
		$insert_sql = "UPDATE `wxch_prize` SET `title` = '$name',`fun` = '$fun',`point`='$point',`loop`= '$loop',`num` = '$num',`starttime` = '$starttime',`endtime` = '$endtime',`dateline` = '$time',`price` = '$price',`is_show` = '$is_show' WHERE `pid` ='$pid';";
		$db->query($insert_sql);
