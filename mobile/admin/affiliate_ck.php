...
elseif ($_REQUEST['act'] == 'separate')
{
    ...
    $row = $db->getRow("SELECT o.*, (o.goods_amount - o.discount) AS goods_amount FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " u ON o.user_id = u.user_id".
            " WHERE order_id = '$oid'");

    $order_sn = $row['order_sn'];
	$user_id=$row['user_id'];
	$userinfo= $db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('users') . 
    ...
    if (empty($row['is_separate']))
    {
        ...
        if(1)
        {
			...
			for ($i=1; $i<=$num; $i++)
            {	
				$up_uid=$userinfo[sales.$i._id];
			
				if(!empty($up_uid)){
					$setmoney=$row[sales.$i._money];
					$setpoint=0;
					$info = sprintf($_LANG['separate_info'], $order_sn, $setmoney, $setpoint);
					log_account_change($up_uid, $setmoney, 0, $setpoint, 0, $info);
					$user_name=$db->getOne("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . 
                    " WHERE user_id = '$up_uid'");
					write_affiliate_log($oid, $up_uid, 	$user_name, $setmoney, $setpoint, $separate_by,$separate_personal); 
				}
			}
...
/*
 批量分成
 */
elseif ($_REQUEST['act'] == 'separate_more')
{
	include_once(ROOT_PATH . 'include/lib_order.php');
    $affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
    empty($affiliate) && $affiliate = array();
    ...
    $oid_list = $_REQUEST['oid'];
    $pieces = explode(",", $oid_list);
    ...
    foreach($pieces as $key=>$val)
    {
	    ...
	    if (empty($row['is_separate']))
	    {
	        ...
	        $integral = integral_to_give(array('order_id' => $val, 'extension_code' => ''));
	        ...
	        if(1)
	        {
	            ...
	            {	
					...
					if(!empty($up_uid)){
						...
						$user_name=$db->getOne("SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . 
	                    " WHERE user_id = '$up_uid'");
						write_affiliate_log($val, $up_uid, 	$user_name, $setmoney, $setpoint, $separate_by,$separate_personal); 
					}
				}
	            ...
	            if ($separate_personal > 0){
	            	...
	            	if($personal_data['rank_points'] >= $level_register_up){
	            		...
	            		$info = sprintf($_LANG['separate_info'], $order_sn, $personalMoney, $personalPoint);
	            		log_account_change($personal_data['user_id'], $personalMoney, 0, $personalPoint, 0, $info);
	            		write_affiliate_log($val, $personal_data['user_id'] , $personal_data['user_name'], $personalMoney, $personalPoint, $separate_by,$separate_personal);
	            	}
	            }
	        }
	        else
	        {
	            ...
	            if(!empty($up_uid) && $up_uid > 0)
	            {
	                $info = sprintf($_LANG['separate_info'], $order_sn, $money, $point);
	                log_account_change($up_uid, $money, 0, $point, 0, $info);
	                write_affiliate_log($val, $up_uid, $row['user_name'], $money, $point, $separate_by);
