...
if ($_REQUEST['act'] == 'list') {
    ...
    make_json_result($smarty->fetch('navigator.htm') , '', array(
        'filter' => $navdb['filter'],
        'page_count' => $navdb['page_count']
    ));
    ...
    if (empty($_REQUEST['step'])) {
        ...
        //上传图片处理
        $file_info = $_FILES['small_img'];
        if(!empty($file_info['tmp_name']))
        {
        	$array_jpg=array('jpeg','jpg','png','gif');
        	$ext = pathinfo($file_info['name'],PATHINFO_EXTENSION);//解析文件后缀
        	if(!in_array($ext, $array_jpg))//后缀是否在$array_jpg中
        	{
        		exit('非法的上传类型');
        	}
        	$max_size = 2097152; //允许上传的最大值 2M
        	...
        	//判断上传目录是否存在
        	// $path = 'mobile/themes/miqinew3/images';

        	$this_path = dirname(dirname(dirname(__FILE__)));
        	$path = $this_path."/mobile/themes/miqinew3/images";
        	if(!file_exists($path))
        	{
        		mkdir($path,0777,true);//创建这个目录，可读可写可执行
        		chmod($path,0777); //chmod — 改变文件权限
        	}
        	//确保文件名唯一，防止重命名产生覆盖
        	$file_name = md5(uniqid(microtime(true),true)).'.'.$ext; //uniqid — 生成一个唯一ID 最后文件名称变成加密后加上文件格式
        	//把临时文件移动到某个文件夹下
        	$move_file = $path.'/'.$file_name;
        	if(!@move_uploaded_file($file_info['tmp_name'],$move_file)) //将服务器上的临时文件移动到指定目录下 成功返回true 否则false
        	{
        		exit('文件移动失败');
        	}
        	$item_pic = $file_name;
        ...
        if ($item_ifshow == 1 && $item_type == 'middle') {
            ...
            if ($arr) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,pic,type) VALUES('$item_name','" . $arr['type'] . "','" . $arr['id'] . "','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_pic',$item_type')";
            }
        }
        if (empty($sql)) {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ifshow,vieworder,opennew,url,pic,type) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_pic','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
            'text' => $_LANG['navigator'],
            'href' => 'navigator.php?act=list'
        );
        $links[] = array(
            'text' => $_LANG['add_new'],
    ...
    if (empty($_REQUEST['step'])) {
        ...
        if ($arr) {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') . " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',pic='$item_pic',type='$item_type' WHERE id='$id'";
        } else {
            if ($row['ctype'] && $row['cid']) {
                set_show_in_nav($row['ctype'], $row['cid'], 0);
            }
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') . " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',pic='$item_pic',type='$item_type' WHERE id='$id'";
        }
        $db->query($sql);
        clear_cache_files();
        $links[] = array(
...
function get_nav() {
    ...
    if ($result === false) {
        ...
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, pic, type" . " FROM " . $GLOBALS['ecs']->table('touch_nav') . " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] . " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
...
function get_sysnav() {
    ...
    $catlist = array_merge(cat_list(0, 0, false) , array(
        '-'
    ) , article_cat_list(0, 0, false));
