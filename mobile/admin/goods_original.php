 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: goods_original.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
$exc = new exchange($ecs->table('goods_original'), $db, 'goods_id', 'goods_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('cat_array', json_encode(cat_list(0, 0, false, 5)));
    ...
    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'goods_original.php?act=list', 'text' => $_LANG['01_goods_list']);
    ...
    $goods_list = goods_original_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1);
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        'goods_original_list.htm' : (($_REQUEST['act'] == 'trash') ? 'goods_trash.htm' : 'goods_original_list.htm');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    {
        /* 商品信息 */
        $sql = "SELECT * FROM " . $ecs->table('goods_original') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods = $db->getRow($sql);
        ...
        /* 商品限购插件 begin */
        $goods['buymax_start_date'] = local_date('Y-m-d H:i', $goods['buymax_start_date']);
        $goods['buymax_end_date'] = local_date('Y-m-d H:i', $goods['buymax_end_date']);
        /* 商品限购插件 end */
...
//var_dump($img_list);exit;
    ...
    $smarty->assign('gd', gd_version_tianxin100());
    ...
    /* 显示商品信息页面 */
    assign_query_info();
    $smarty->display('goods_original_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods_original') .
                " WHERE goods_sn = '$_REQUEST[goods_sn]' AND is_delete = 0 AND goods_id <> '$_REQUEST[goods_id]'";
        if ($db->getOne($sql) > 0)
        {
    ...
    // 如果上传了商品图片，相应处理
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_REQUEST['goods_img_url'] != $_LANG['lab_picture_url'] && $_REQUEST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        if ($_REQUEST['goods_id'] > 0)
        {
            /* 删除原来的图片文件 */
            $sql = "SELECT goods_thumb, goods_img, original_img " .
                    " FROM " . $ecs->table('goods_original') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
            $row = $db->getRow($sql);
        ...
        elseif (copy(trim($_REQUEST['goods_img_url']), ROOT_PATH . 'data/' . basename($_REQUEST['goods_img_url'])))
        {
            $original_img = 'data/' . basename($_REQUEST['goods_img_url']);
        }
        ...
        if ($_CFG['auto_generate_gallery'])
        {
            ...
            if (!copy(WEB_PATH.$img, WEB_PATH.$newname))
            {
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if (empty($is_url_goods_img))
            {
                ...
                {
                    $goods_img = $image->make_thumb(WEB_PATH. $goods_img , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                    if ($goods_img === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
                ...
                {
                    if ($image->add_watermark(WEB_PATH.$goods_img,'',$GLOBALS['_CFG']['watermark'], $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']) === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
                    /* 添加判断是否自动生成相册图片 */
                    if ($_CFG['auto_generate_gallery'])
                    {
                        if ($image->add_watermark(WEB_PATH. $gallery_img,'',$GLOBALS['_CFG']['watermark'], $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']) === false)
                        {
                            sys_msg($image->error_msg(), 1, array(), false);
                        }
                    }
            ...
            if ($_CFG['auto_generate_gallery'])
            {
                ...
                {
                    $gallery_thumb = $image->make_thumb(WEB_PATH . $img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_REQUEST['auto_thumb']) && !empty($original_img))
        {
            // 如果设置缩略图大小不为0，生成缩略图
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                $goods_thumb = $image->make_thumb(WEB_PATH . $original_img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
                if ($goods_thumb === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    if (!empty($is_url_goods_img))
    {
        unlink(WEB_PATH . $original_img);
        empty($newname) || unlink(WEB_PATH . $newname);
        $url_goods_img = $goods_img = $original_img = htmlspecialchars(trim($_REQUEST['goods_img_url']));
    }
    ...
    /* 处理商品数据 */
    $shop_price = !empty($_REQUEST['shop_price']) ? $_REQUEST['shop_price'] : 0;
    $market_price = !empty($_REQUEST['market_price']) ? $_REQUEST['market_price'] : 0;
    $promote_price = !empty($_REQUEST['promote_price']) ? floatval($_REQUEST['promote_price'] ) : 0;
    $is_promote = empty($promote_price) ? 0 : 1;
    $promote_start_date = ($is_promote && !empty($_REQUEST['promote_start_date'])) ? local_strtotime($_REQUEST['promote_start_date']) : 0;
    $promote_end_date = ($is_promote && !empty($_REQUEST['promote_end_date'])) ? local_strtotime($_REQUEST['promote_end_date']) : 0;
    $goods_weight = !empty($_REQUEST['goods_weight']) ? $_REQUEST['goods_weight'] * $_REQUEST['weight_unit'] : 0;
    $is_best = isset($_REQUEST['is_best']) ? 1 : 0;
    $is_new = isset($_REQUEST['is_new']) ? 1 : 0;
    $is_hot = isset($_REQUEST['is_hot']) ? 1 : 0;
    $is_on_sale = isset($_REQUEST['is_on_sale']) ? 1 : 0;
    $is_alone_sale = isset($_REQUEST['is_alone_sale']) ? 1 : 0;
    $is_shipping = isset($_REQUEST['is_shipping']) ? 1 : 0;
    $goods_number = isset($_REQUEST['goods_number']) ? $_REQUEST['goods_number'] : 0;
    $warn_number = isset($_REQUEST['warn_number']) ? $_REQUEST['warn_number'] : 0;
    $goods_type = isset($_REQUEST['goods_type']) ? $_REQUEST['goods_type'] : 0;
    $give_integral = isset($_REQUEST['give_integral']) ? intval($_REQUEST['give_integral']) : '-1';
	$fencheng = isset($_REQUEST['fencheng']) ? $_REQUEST['fencheng'] : '0';
    $rank_integral = isset($_REQUEST['rank_integral']) ? intval($_REQUEST['rank_integral']) : '-1';
    $suppliers_id = isset($_REQUEST['suppliers_id']) ? intval($_REQUEST['suppliers_id']) : '0';

    $goods_name_style = $_REQUEST['goods_name_color'] . '+' . $_REQUEST['goods_name_style'];

    $catgory_id = empty($_REQUEST['cat_id']) ? '' : intval($_REQUEST['cat_id']);
    $brand_id = empty($_REQUEST['brand_id']) ? '' : intval($_REQUEST['brand_id']);

    $goods_thumb = (empty($goods_thumb) && !empty($_REQUEST['goods_thumb_url']) && goods_parse_url($_REQUEST['goods_thumb_url'])) ? htmlspecialchars(trim($_REQUEST['goods_thumb_url'])) : $goods_thumb;
    $goods_thumb = (empty($goods_thumb) && isset($_REQUEST['auto_thumb']))? $goods_img : $goods_thumb;
    $special_ico = $_REQUEST['special_ico'];
    $goods_guide = $_REQUEST['goods_guide'];
    $commission_able = isset($_REQUEST['commission_able']) ? $_REQUEST['commission_able'] : 1;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods_original') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, goods_guide,commission_able," .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,fencheng, suppliers_id, special_ico)" .
                "VALUES ('$_REQUEST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', '$goods_guide', '$commission_able',".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_REQUEST[keywords]', '$_REQUEST[goods_brief]', '$_REQUEST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_REQUEST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_REQUEST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$fencheng', '$suppliers_id', '$special_ico')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods_original') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,fencheng, special_ico)" .
                "VALUES ('$_REQUEST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', '$goods_guide', '$commission_able',".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_REQUEST[keywords]', '$_REQUEST[goods_brief]', '$_REQUEST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_REQUEST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_REQUEST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$fencheng', '$special_ico')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods_original') . " SET " .
        		...
        		"special_ico = '$special_ico', " .
        		"goods_guide = '$goods_guide', " .
        		"commission_able = '$commission_able', ";
        ...
        $sql .= "keywords = '$_REQUEST[keywords]', " .
                ...
                "seller_note = '$_REQUEST[seller_note]', " .
    ...
    if ($is_insert)
    {
        admin_log($_REQUEST['goods_name'], 'add', 'goods');
    }
    else
    {
        admin_log($_REQUEST['goods_name'], 'edit', 'goods');
    }

    /* 处理属性 */
    if ((isset($_REQUEST['attr_id_list']) && isset($_REQUEST['attr_value_list'])) || (empty($_REQUEST['attr_id_list']) && empty($_REQUEST['attr_value_list'])))
    {
        // 取得原有的属性值
        $goods_attr_list = array();

        $keywords_arr = explode(" ", $_REQUEST['keywords']);
        ...
        // 循环现有的，根据原有的做相应处理
        if(isset($_REQUEST['attr_id_list']))
        {
            foreach ($_REQUEST['attr_id_list'] AS $key => $attr_id)
            {
                $attr_value = $_REQUEST['attr_value_list'][$key];
                $attr_price = $_REQUEST['attr_price_list'][$key];
                if (!empty($attr_value))
                {
        ...
        $sql = "UPDATE " .$ecs->table('goods_original'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
    ...
    /* 处理会员价格 */
    if (isset($_REQUEST['user_rank']) && isset($_REQUEST['user_price']))
    {
        handle_member_price($goods_id, $_REQUEST['user_rank'], $_REQUEST['user_price']);
    }
    ...
    /* 处理扩展分类 */
    if (isset($_REQUEST['other_cat']))
    {
        handle_other_cat($goods_id, array_unique($_REQUEST['other_cat']));
    }
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_original') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_original') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods_original') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    ...
    handle_gallery_image($goods_id, $_FILES['img_url'], $_REQUEST['img_desc'], $_REQUEST['img_file']);

    /* 编辑时处理相册图片描述 */
    if (!$is_insert && isset($_REQUEST['old_img_desc']))
    {
        foreach ($_REQUEST['old_img_desc'] AS $img_id => $img_desc)
        {
    ...
    {
        $db->query("UPDATE " . $ecs->table('goods_original') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
    ...
    /* 商品限购插件 begin */
    $buymax = $_REQUEST['buymax'] ? intval($_REQUEST['buymax']) : 0;
    $buymax_start_date = ($buymax && !empty($_REQUEST['buymax_start_date'])) ? local_strtotime($_REQUEST['buymax_start_date']) : 0;
    $buymax_end_date = ($buymax && !empty($_REQUEST['buymax_end_date'])) ? local_strtotime($_REQUEST['buymax_end_date']) : 0;
    $db->query("UPDATE " . $ecs->table('goods') . " SET buymax='$buymax', buymax_start_date = '$buymax_start_date' , buymax_end_date = '$buymax_end_date' WHERE `goods_id`='{$goods_id}'");
    /* 商品限购插件 end */
    ...
    {
        $link[0] = array('href' => 'goods_original.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    /* 取得要操作的商品编号 */
    $goods_id = !empty($_REQUEST['checkboxes']) ? join(',', $_REQUEST['checkboxes']) : 0;
    ...
    if (isset($_REQUEST['type']))
    {
        ...
        /* 下架 */
        elseif ($_REQUEST['type'] == 'not_on_sale')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
        ...
        /* 转移到分类 */
        elseif ($_REQUEST['type'] == 'move_to')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'cat_id', $_REQUEST['target_cat']);
        }

        /* 转移到分类 */
        elseif ($_REQUEST['type'] == 'add_to')
        {
        	/* 检查权限 */
        	admin_priv('goods_manage');
        	add_other_cat($goods_id, $_REQUEST['target_cat']);
        }

        /* 转移到供货商 */
        elseif ($_REQUEST['type'] == 'suppliers_move_to')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'suppliers_id', $_REQUEST['suppliers_id']);
        }

        /* 还原 */
        elseif ($_REQUEST['type'] == 'restore')
        {
            /* 检查权限 */
            admin_priv('remove_back');
    ...
    if ($_REQUEST['type'] == 'drop' || $_REQUEST['type'] == 'restore')
    {
        $link[] = array('href' => 'goods_original.php?act=trash', 'text' => $_LANG['11_goods_trash']);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'edit_goods_name')
{
    ...
    $goods_name = json_str_iconv(trim($_REQUEST['val']));
...
elseif ($_REQUEST['act'] == 'edit_goods_sn')
{
    ...
    $goods_id = intval($_REQUEST['id']);
    $goods_sn = json_str_iconv(trim($_REQUEST['val']));
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
    ...
    $goods_id       = intval($_REQUEST['id']);
    $goods_price    = floatval($_REQUEST['val']);
    $price_rate     = floatval($_REQUEST['market_price_rate'] * $goods_price);

    if ($goods_price < 0 || $goods_price == 0 && $_REQUEST['val'] != "$goods_price")
    {
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
    ...
    $goods_id   = intval($_REQUEST['id']);
    $goods_num  = intval($_REQUEST['val']);

    if($goods_num < 0 || $goods_num == 0 && $_REQUEST['val'] != "$goods_num")
    {
...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    $goods_id       = intval($_REQUEST['id']);
    $sort_order     = intval($_REQUEST['val']);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $goods_list = goods_original_list($is_delete, ($code=='') ? 1 : 0);
    ...
    $tpl = $is_delete ? 'goods_trash.htm' : 'goods_original_list.htm';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
        ...
        $url = 'goods_original.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'edit_sales_volume_base')
{
    ...
    $goods_id = intval($_REQUEST['id']);
    $sales_volume_base = json_str_iconv(trim($_REQUEST['val']));
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
    ...
    $url = 'goods_original.php?act=query&' . str_replace('act=restore_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    clear_cache_files();
    $url = 'goods_original.php?act=query&' . str_replace('act=drop_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    {
        $link[] = array('href' => 'goods_original.php?act=list', 'text' => $_LANG['cannot_found_goods']);
    ...
    if (empty($goods))
    {
        $link[] = array('href' => 'goods_original.php?act=list', 'text' => $_LANG['01_goods_list']);
        sys_msg($_LANG['cannot_found_goods'], 1, $link);
    }
    ...
    if (empty($attribute))
    {
        $link[] = array('href' => 'goods_original.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['edit_goods']);
    ...
    $smarty->assign('action_link',  array('href' => 'goods_original.php?act=list', 'text' => $_LANG['01_goods_list']));
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    if ($result)
    {
        ...
        $url = 'goods_original.php?act=product_query&' . str_replace('act=product_remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'edit_product_sn')
{
    ...
    $product_id       = intval($_REQUEST['id']);
    $product_sn       = json_str_iconv(trim($_REQUEST['val']));
...
elseif ($_REQUEST['act'] == 'edit_product_number')
{
    ...
    $product_id       = intval($_REQUEST['id']);
    $product_number       = intval($_REQUEST['val']);
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['goods_id']        = intval($_REQUEST['goods_id']);
    $product['attr']            = $_REQUEST['attr'];
    $product['product_sn']      = $_REQUEST['product_sn'];
    $product['product_number']  = $_REQUEST['product_number'];
    ...
    if ($insert)
    {
         $link[] = array('href' => 'goods_original.php?act=add', 'text' => $_LANG['02_goods_add']);
         $link[] = array('href' => 'goods_original.php?act=list', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'goods_original.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
    else
    {
         $link[] = array('href' => 'goods_original.php?act=list&uselastfilter=1', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'goods_original.php?act=edit&goods_id=' . $product['goods_id'], 'text' => $_LANG['edit_goods']);
         $link[] = array('href' => 'goods_original.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
...
{
    /* 定义返回 */
    $link[] = array('href' => 'goods_original.php?act=product_list&goods_id=' . $_REQUEST['goods_id'], 'text' => $_LANG['item_list']);
    ...
    if ($_REQUEST['type'] == 'drop')
    {
        ...
        //取得要操作的商品编号
        $product_id = !empty($_REQUEST['checkboxes']) ? join(',', $_REQUEST['checkboxes']) : 0;
...
{
    $href = 'goods_original.php?act=list';
...
{
    $href = 'goods_original.php?act=add';
    if (!empty($extension_code))
    {
        $href .= '&extension_code=' . $extension_code;
    }
