...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $shipping = $db->getRow("SELECT shipping_name, shipping_code FROM " .$ecs->table('touch_shipping'). " WHERE shipping_id='$_REQUEST[shipping]'");

    $set_modules = 1;
    include_once(ROOT_PATH . 'include/modules/shipping/'.$shipping['shipping_code'].'.php');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$ecs->table("touch_shipping_area").
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$ecs->table('touch_shipping').
                                    " WHERE shipping_id='$_POST[shipping]'");
        $plugin        = '../include/modules/shipping/'. $shipping_code. ".php";
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                $sql = "INSERT INTO ".$ecs->table('touch_area_region')." (shipping_area_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ".
            "FROM " .$ecs->table('touch_shipping'). " AS a, " .$ecs->table('touch_shipping_area'). " AS b ".
...
//       }
    ...
    $sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$ecs->table('touch_area_region')." AS a, ".$ecs->table('region'). " AS r ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$ecs->table('touch_shipping'). " WHERE shipping_id='$_POST[shipping]'");
        $plugin        = '../include/modules/shipping/'. $shipping_code. ".php";
        ...
        $sql = "UPDATE " .$ecs->table('touch_shipping_area').
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$ecs->table("touch_area_region")." WHERE shipping_area_id='$_POST[id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$ecs->table('touch_area_region')." (shipping_area_id, region_id) VALUES ('$_POST[id]', '$val')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
    ...
    if (isset($_POST['areas']) && count($_POST['areas']) > 0)
    {
        ...
        {
            $db->query("DELETE FROM " .$ecs->table('touch_shipping_area'). " WHERE shipping_area_id='$v'");
            $i++;
        }
...
elseif ($_REQUEST['act'] == 'remove_area')
{
    ...
    $exc->drop($id);
    $db->query('DELETE FROM '.$ecs->table('touch_area_region').' WHERE shipping_area_id='.$id);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('touch_shipping_area');
    if ($shipping_id > 0)
    {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = "SELECT r.region_name " .
                "FROM " . $GLOBALS['ecs']->table('touch_area_region'). " AS a, " .
