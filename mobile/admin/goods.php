...
$logger = LoggerManager::getLogger('goods.php');
...
//var_dump($img_list);exit;
    ...
    if($goods['cat_id']){
    	$cat_name = $db->getOne("select cat_name from " . $ecs->table('category') . " where cat_id='$goods[cat_id]'");
    	$logger->debug("select cat_name from " . $ecs->table('category') . " where cat_id='$goods[cat_id]'");
...
{
    /* 定义返回 */
    $link[] = array('href' => 'goods.php?act=product_list&goods_id=' . $_REQUEST['goods_id'], 'text' => $_LANG['item_list']);
