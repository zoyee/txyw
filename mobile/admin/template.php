...
if ($_REQUEST['act'] == 'setup')
{
    ...
    $sql = "SELECT region, library, sort_order, id, number, type, display FROM ".$ecs->table('touch_template') ." ".
    ...
    while ($row= $db->FetchRow($rc))
    {
        ...
        else
        {
            ...
            if (isset($lib))
            {
                ...
                $temp_options[$lib]['sort_order'] = $row['sort_order'];
                $temp_options[$lib]['display'] = $row['display'];
            }
        }
    }
    ...
    foreach ($_POST['regions'] AS $key => $val)
    {
        ...
        if (!in_array($key, $GLOBALS['dyna_libs']) AND (isset($_POST['display'][$key]) AND $_POST['display'][$key] == 1 OR $number > 0))
        {
            $sql = "INSERT INTO " .$ecs->table('touch_template').
                        "(theme, filename, region, library, sort_order, number, display)".
                    " VALUES ".
                        "('$curr_template', '$_POST[template_file]', '$val', '".$_POST['map'][$key]."', '" . @$_POST['sort_order'][$key] . "', '$number', '" . @$_POST['display'][$key] . "')";
            $db->query($sql);
        }
    }
...
if ($_REQUEST['act'] == 'install')
{
    ...
    $sql = "UPDATE " .$GLOBALS['ecs']->table('touch_shop_config'). " SET value = '$tpl_name' WHERE code = 'template'";
    $step_one = $db->query($sql, 'SILENT');
    $sql = "UPDATE " .$GLOBALS['ecs']->table('touch_shop_config'). " SET value = '$tpl_fg' WHERE code = 'stylename'";
...
if ($_REQUEST['act'] == 'backup_setting')
{
    ...
    $sql = "SELECT DISTINCT(remarks) FROM " . $ecs->table('touch_template') . " WHERE theme = '" . $_CFG['template'] . "' AND remarks > ''";
    $col = $db->getCol($sql);
    $remarks = array();
    foreach ($col as $val)
    {
    ...
    $sql = "SELECT DISTINCT(filename) FROM " . $ecs->table('touch_template') . " WHERE theme = '" . $_CFG['template'] . "' AND remarks = ''";
    $col = $db->getCol($sql);
    $files = array();
    foreach ($col as $val)
    {
...
if ($_REQUEST['act'] == 'act_backup_setting')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('touch_template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['template'] . "'";
    if ($db->getOne($sql) > 0)
    {
...
if ($_REQUEST['act'] == 'del_backup')
{
    ...
    if ($remarks)
    {
        $sql = "DELETE FROM " . $ecs->table('touch_template') . " WHERE remarks='$remarks' AND theme = '" . $_CFG['template'] . "'";
        $db->query($sql);
    }
