...
/**
 * 添加扩展分类
 * @param array $goods_id_list 商品ID数组
 * @param string $cat_id 分类ID
 */
function add_other_cat($goods_id_list, $cat_id)
{
	/* 查询现有的扩展分类 */
	$sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_cat') .
	" WHERE goods_id " . db_create_in($goods_id_list) . " and cat_id=$cat_id";
	$exist_list = $GLOBALS['db']->getAll($sql);

	$goods_id_arr = split(',', $goods_id_list);
	$exist_arr = array();
	foreach ($exist_list as $row){
		array_push($exist_arr, $row['goods_id']);
	}
	foreach ($goods_id_arr as $goods_id){
		if($exist_list){
			if(!in_array($goods_id, $exist_arr)){
				$sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_cat') .
					" (goods_id, cat_id) " .
					"VALUES ('$goods_id', '$cat_id')";
				$GLOBALS['db']->query($sql);
...
function goods_original_list($is_delete, $real_goods=1, $conditions = '')
{
    /* 过滤条件 */
    $param_str = '-' . $is_delete . '-' . $real_goods;
    $result = get_filter($param_str);
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods_original'). " AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, sales_volume_base, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods_original') . " AS g WHERE is_delete='$is_delete' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ";
        if($filter['sort_by'] != 'goods_id'){
        	$sql .= " , goods_id desc";
        }
