...
$logger = LoggerManager::getLogger('user_bonus.php');
...
elseif ($_REQUEST['act'] == 'ctadd')
{
    ...
    //查询特殊会员等级，可以绑定会员等级实现折扣
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('user_rank') . " where special_rank=1 order by rank_id desc";
	$user_rank_list = $db->getAll($sql);
	$smarty->assign('user_rank_list', $user_rank_list);
	
    //查询红包类型，可以赠送红包
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('bonus_type') . " order by type_id desc";
...
elseif ($_REQUEST['act'] == 'ctinsert')
{
   ...
   $arr = array();
	...
	$arr['bonus_type'] = $_POST['bonus_type'];
	$arr['user_rank'] = $_POST['user_rank'];
...
elseif ($_REQUEST['act'] == 'send')
{
	...
	if(isset($_POST['excel']) && !empty($_POST['excel']))//电子表格批量导入
	{
    ...
    sys_msg($_LANG['batch_upload_ok'], 0, $link);
	   ...
	   if($_POST['batch'])//批量生产会员卡
	   {
		   ...
		   $card_user_rank = $db->getOne("select user_rank from " . $ecs->table('user_card_type') . " where ct_id='$ct_id'");
		   ...
		   for ($i=1;$i<=$num;$i++)
		   {
				...
				$arr['card_pass'] = generate_password($pass_len);
		   		...
		   		$param = array(
		   			"user_card" => $arr['card_no'],
		   			"new_user_limit" => 1,
		   			"scan_limit" => 1		//二维码只能扫描使用一次
				);
				if($card_user_rank){
					$param['user_rank'] = $card_user_rank;
				}
				if($bonus_type){
					$param['bonus_type'] = $bonus_type;
				}
				$param = json_encode($param);
				$logger->debug('$param = ' . $param);
				$createtime = date('Y-m-d H:i:s');
				$endtime = date('Y-m-d H:i:s', time() + 2592000);
				$sql = "insert into wxch_scene (`type`,param, createtime, endtime) values (1, '$param','$createtime', '$endtime')";
				$logger->debug($sql);
				...
				$qr_code = $weixin_api->cr_qrcode($json_arr, "user_card");
				$logger->debug(json_encode($qr_code));
				if($qr_code->errcode == 0){
					$qr_path = ROOT_PATH . $qr_code->data;
					$GLOBALS['db'] -> query("update wxch_scene set ticket='" . $qr_code->ticket . "' where id=$scene_id");
					//重命名二维码图片文件
					$base_name = basename($qr_path, ".jpg");
					$new_path = str_replace($base_name, $arr['card_no'], $qr_path);
					rename($qr_path, $new_path);
					$qr_path = str_replace(ROOT_PATH, "", $new_path);
					$arr['qr_code'] = $qr_path;
					$logger->debug('$arr = ' . json_encode($arr));
				}
				if ($exc->is_only('card_no', $arr['card_no']))
				{
					 $db->autoExecute($ecs->table('user_card'), $arr, 'INSERT');
