...
// $logger = LoggerManager::getLogger('pintuan.php');
...
if ($_REQUEST['act'] == 'asynclist')
{
    ...
    if (is_array($goodslist)) {
        ...
        foreach ($goodslist as $vo) {
            ...
            $sayList[] = array(
                'pro-inner' => '<a href="' . $vo['url'] . '"><div class="product flex_in single_item" id="more_element_'.$i.'">
            <div class="pro-inner">
            <div class="proImg-wrap" style="width: 36%;"> <img alt="' . $vo['goods_name'] . '" src="' . $img_url . '"></div>
            <div class="proInfo-wrap">
            <div class="proTitle">' . $vo['act_name'] . '</div>
            <div class="proSKU"></div>
            <div class="proPrice" style="color:red">'.$vo['lowest_price'].'</div>
            <div class="proService"><del>' . "¥".$vo['market_price'] . '</del></div>
            <div class="proSales">销量:<em>'.selled_count($vo['goods_id']).'</em></div>
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('pintuan_goods.dwt', $cache_id))
    {
        ...
        $pintuan['virtual_sold'] =selled_count($goods_id);//$pintuan['virtual_sold']+$goods['sales_count'] ;
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    {
    	ecs_header("Location: subscribe.php?pintuan_id=$pt_id\n");
    	exit;
...
//         $logger->debug(json_decode($product_info));
 ...
 * 限购检验
 */
elseif($_REQUEST['act'] == 'check_limit'){
	include_once('include/cls_json.php');
	$json  = new JSON;
	...
	$sql = "select buymax from ". $GLOBALS['ecs']->table('goods').
		" where goods_id='$goods_id' and buymax_start_date<=$time and $time < buymax_end_date + 86400";
	...
	if($limit){
		...
		if($num > $limit){
			$result['error'] = 888;
			$result['message'] = "此商品为限购商品，活动期间每单限购".$limit."件！";
			die($json->encode($result));
		}
	}
	die($json->encode($result));
