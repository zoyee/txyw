...
if (empty($_REQUEST['act']))
{
    ...
    {
    	$result['error']   = 1;
    	$result['message'] =  "请先登录再评论商品";
    } 
    elseif(!empty($member_id))
    {
    	
    	 $sql_or_buy = "SELECT mobile FROM " . $ecs->table('order_info') . 
    	       " t1 left join ".$ecs->table('order_goods') .
    	" t2 on t1.order_id = t2.order_id where t1.user_id = $member_id and t2.goods_id = $good_id"; 
    	$or_buy_good = $db->getOne($sql_or_buy);  //getOne($sql_or_buy); 
    	if(empty($or_buy_good))
    	{
    		$result['error']   = 1;
    		$result['message'] =  "请先购买该商品再评论";
    	}else{
    		$email = substr($or_buy_good , 0 , 3)."*****".substr($or_buy_good , 8 );    	
    		$cmt->email = $email;
    	}
    }
    else
...
if ($result['error'] == 0)
{
	$logger = LoggerManager::getLogger('comment.php');
	$logger->debug("评论1");
