...
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','send_pwd_sms','password', 'signin', 'add_tag',
    ...
    'get_passwd_question', 'check_answer', 'oath', 'oath_login','tracking_goback');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile','dianpu', 'act_dianpu', 'order_list', 'order_detail','order_detail_liangwei1', 'order_tracking', 'address_list', 'act_edit_address', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply','tracking_goback',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','point','user_card','fenxiao1','myorder','myorder_detail','fenxiao2','fenxiao3','fenxiao4','tuiguang');
/* 未登录处理 */
...
if ($action == 'default')
{
	...
	{
		if($rank_points>=$val['min_points']&&$rank_points<$val['max_points'])
		{
			$info['rank_name'] = $val['rank_name'];
		}
	}
	...
	$smarty->assign('sur_amount',   $sur_amount);
    ...
    $smarty->assign('affiliate',     $affiliate['on']);
...
if ($action == 'act_dianpu')
{
    ...
    $smarty->display('change_shop_name.dwt');
}
//推广二维码
elseif($action == 'tuiguang')
{
	$qr_path = $GLOBALS['db']->getRow("SELECT qr_path FROM wxch_qr_tianxin100 WHERE scene_id = '$user_id'");
...
{
	//登录改为扫码登录
	ecs_header("Location: subscribe.php\n");
	exit;
...
elseif ($action == 'profile')
{
    ...
    $weixinInfo = $GLOBALS['db']->getRow("SELECT nickname FROM wxch_user WHERE uname = '$user_name'");
    
    //echo ':'.$user_info['wxid'].':<br/>';
    if(!empty($user_info['wxid'])){
    	$weixinInfo = $GLOBALS['db']->getRow("SELECT nickname, headimgurl, subscribe_time FROM wxch_user WHERE wxid = '".$user_info['wxid']."'");
    	$user_info['avatar'] = empty($weixinInfo['headimgurl']) ? '':$weixinInfo['headimgurl'];
    	$user_info['nickname'] = $weixinInfo['nickname'];
    	$user_info['subscribe_time'] = date("Y-m-d",$weixinInfo['subscribe_time']);
    }
    //echo var_dump($user_info);
    /* 密码提示问题 */
    $smarty->assign('passwd_questions', $_LANG['passwd_questions']);
    $smarty->assign('nickname', $weixinInfo[nickname]);
...
elseif ($action == 'act_edit_profile')
{
    ...
    /* if ($user->check_user($username) || admin_registered($username))
    {
    	show_message('用户名 '.$username.'已经存在1');
    }
    else
    {
    	show_message('用户名 '.$username.'已经存在2');
    } */
    $user_info = get_profile($user_id);
//    $user_name =$user_info['user_name'];
//    $updatesql = "update wxch_user set nickname = '$username' where uname = '$user_name'";
//    $db->query($updatesql);
    
   	$mobilenew = $_POST['mobile'];
    ...
    $other['home_phone'] = isset($_POST['extend_field4']) ? trim($_POST['extend_field4']) : ''; 
    
    $sel_question = empty($_POST['sel_question']) ? '' : compile_str($_POST['sel_question']);
    $passwd_answer = isset($_POST['passwd_answer']) ? compile_str(trim($_POST['passwd_answer'])) : '';
	if($mobilenew){
		$other['mobile_phone']  = $mobilenew;
	}
	if($weixin){
		$other['wxh'] = $weixin;
	}
    /* 更新用户扩展字段的数据   $home_phone =*/
    ...
    if (edit_profile($profile))
    {
    	...
    	if($_POST['isVerify'] && $_POST['verify_code']){
	    	$verifycode = $_POST['verify_code'];
	    	$sql = "SELECT COUNT(id) FROM " . $ecs->table('verifycode') ." WHERE mobile='$mobilenew' AND verifycode='$verifycode' AND status=4 AND dateline>'" . gmtime() ."'-".$_CFG['ecsdxt_smsgap'];
	    	$logger->debug($sql);
	    	if ($db->getOne($sql) == 0){
	    		show_message($_LANG['verifycode_mobile_phone_notmatch'], $_LANG['profile_lnk'], 'user.php?act=profile', 'info');
	    	}
	    	...
	    	if($mobilenew)
	    	{
	    		$sql = 'UPDATE ' . $ecs->table('users') . " SET `mobile_phone`='$mobilenew'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
	       		$db->query($sql);
	    	}
    	}
...
elseif ($action == 'async_order_list')
{
    ...
    $start = $_POST['last'];
    $limit = $_POST['amount'];
    $limit = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id'");
    ...
    if(is_array($orders)){
        foreach($orders as $vo){
            ...
            $tracking = ($vo['shipping_id'] > 0) ?  "<a href=\"user.php?act=order_tracking&order_id=" .$vo['order_id'].
                	"\" ><button style=\"width: 70px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid rgb(213, 77,43); height: 2rem; border-radius: 0.2rem; color: rgb(213, 77, 43); margin-right: 10px;\">订单跟踪</button></a>":'';
            
            //'<a href="user.php?act=order_tracking&order_id='.$vo['order_id'].'" class="c-btn3">订单跟踪</a>'
            
            $goodsNum = $db->getOne("select sum(goods_number) from " .$ecs->table('order_goods'). " where  order_id = ".$vo['order_id']);
            
            $button = "";

            if ($vo['order_status'] == '已确认,已付款,收货确认' || $vo['order_status'] == '已确认,已付款,未发货' ) {//|| $vo['order_status'] == '已确认,已付款,配货中'
                $is_separate = $db->getOne("SELECT is_separate FROM " .$ecs->table('order_info')." where order_id = ".$vo['order_id']);
				 if($is_separate){
					$back_order = '';
				}else{
					//$back_order = '<a href="user.php?act=back_order&order_id='.$vo['order_id'].'" class="c-btn5">申请退款</a>';	
					$back_order = "<a href=\"user.php?act=back_order&order_id=" .$vo['order_id'].
                	"\" ><button style=\"width: 70px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">申请退款</button></a>";			
				}
				//$comments = '<a href="user.php?act=order_comments&order_id='.$vo['order_id'].'" class="c-btn6" >评论</a>';
				$comments =  "<a href=\"user.php?act=order_comments&order_id=" .$vo['order_id'].
                	"\" ><button style=\"width: 3.5rem; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">评论</button></a>";	
            }else if($vo['order_status'] == '已确认,已付款,已发货'){
                //$back_order = '<a href="user.php?act=back_order&order_id='.$vo['order_id'].'" class="c-btn5">申请退款</a>';
            	$back_order = "<a href=\"user.php?act=back_order&order_id=" .$vo['order_id'].
            	"\" ><button style=\"width: 70px; background-color: rgb(255, 255, 255); font-size: 14px; border: 1px solid #000; height: 2rem; border-radius: 0.2rem; color: #000; margin-right: 10px;\">申请退款</button></a>";
                //$comments = '<a href="user.php?act=order_comments&order_id='.$vo['order_id'].'" class="c-btn6" >评论</a>';
            ...
            //$back_order = '';
            $button = $vo['handler'].$back_order.$comments;
            $asyList[] = array(
                'order_status' => '订单状态：'.$vo['order_status'],
                'order_handler' => $vo['handler'],
                'order_content' => '<a href="user.php?act=order_detail&order_id='.$vo['order_id'].'">
                <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table_no_border">
            	<tr>
                <td width="85px" style="padding-left: 0px; padding-right: 0px;">
                	<img src="'.$config['site_url'].$img.'" width="75px" style="border: 1px solid #e8e8e8;" />
                </td>
                <td style="line-height: 25px;color: rgb(51, 51, 51);">
                	订单编号：'.$vo['order_sn'].'<br>
            		快递单号：'.$vo['invoice_no'].'<br>
                	下单时间：'.$vo['order_time'].'</td>
                <td width="10px" style="position:relative"><span class="new-arr"></span></td>
            </tr>
          </table></a>',
            	'button' => $button,
            	'goodsNum' =>'共 <strong>'.$goodsNum.'</strong> 件商品  实付款：<span class="price">'.$vo['total_fee'].'</span>',
...
elseif ($action == 'back_order')
{
    ...
    $orders = $db->getRow("SELECT order_id,order_sn,money_paid FROM " .$ecs->table('order_info'). " WHERE order_id = $order_id");
...
/* 处理申请退款数据 */
elseif ($action == 'do_back')
{
    $order_id = $_POST['order_id'];
    $back_reason = $_POST['back_reason'];

    $sql = "update ".$GLOBALS['ecs']->table('order_info')." set order_status = 7 ,back_reason = '$back_reason' where order_id = ".$order_id;
...
elseif ($action == 'order_tracking')
{
    ...
    // split($orders['invoice_no'], ',');
    //echo $orders['invoice_no'];
    ...
    //echo var_dump($shipping)."<br>";
    for($i = 0;$i < count($invoiceList);$i++)
    {
    	//生成快递100查询接口链接
    	//echo $invoiceList[$i]."<br>";
    	$query_link = $shipping->kuaidi100($invoiceList[$i]);
    	$query_link = str_replace('id=&',"id=9863ccbbbe86a7def58ec03c843f51a9&",$query_link);
    	//echo $query_link."<br>";
    	//优先使用curl模式发送数据
    	/* if (function_exists('curl_init') == 1){
    	 ...
    	 $logger->debug('查询物流$query_link = ' . $query_link);
    	 $get_content = curl_exec($curl);
    	 curl_close ($curl);
    	 } */
    	for ($x=0; $x<=3; $x++) {
    		$get_content = file_get_contents($query_link);
    		$json = json_decode($get_content, true);
    		if($json['success']) break;
    	}
    	//echo $get_content."<br>";
    	//$aracon[$i] = $get_content;
    	$json = json_decode($get_content, true);
    	//var_dump($json);
    	if(!$json['success']){
    		$content = $content."<tr><td>老农温馨提示：您的订单快递小哥暂时还未同步信息到快递官网，请耐心等待，稍后再查询一下喔O(∩_∩)O~</td></tr>";
    		...
    		for($n = 0;$n<count($data);$n++){
    			...
    			}else if($n == count($data) - 1){
    				$className =$className." first-line";
    			}else if($n == 0){
    				$className =$className." last-line";
    				if($data['ischeck'] == 1){
    					$className =$className." checked";
    				}else{
    					$className =$className." wait";
    			...
    			$index = strpos($data[$n]['time']," ");
    			$result_date = substr($data[$n]['time'],0,$index);
    			$result_time = substr($data[$n]['time'],$index+1);
    			$s_index = strrpos($result_time,":");
    			$result_time = substr($result_time,0,$s_index);
    			//echo $result_time;
    			$kddh = "";
    			if($n==0){
    				$kddh = "<br>快递单号：".$invoiceList[$i];
    			}
    			$content = $content. "<tr class='$className'><td class='col1'><span class='result-date'>$result_date</span><span class='result-time'>$result_time</span></td><td class='colstatus'></td><td class='col2'><span>".$data[$n]["context"].$kddh."</span></td></tr>";
    			
    			//$index = $data[$n].time.indexOf(" ");
    		}
    	}
    }
   ...
   //echo $content;
    ...
    $smarty->assign('shipping_name', $orders['shipping_name']);
    $smarty->assign('invoice_no', $orders['invoice_no']);
//     $smarty->assign('shipping_name', '快递');
//     $smarty->assign('invoice_no', '778731129527');
    $smarty->assign('trackinfo',$content);
    $smarty->display('user_transaction.dwt');
}
elseif($action=="tracking_goback"){
	$smarty->display('tracking_goback.dwt');
}
...
/*--------------------------------------- 测试订单支付--------------------------------------*/
elseif ($action == 'order_detail_liangwei1')
{
    include_once(ROOT_PATH . 'include/lib_transaction.php');
    include_once(ROOT_PATH . 'include/lib_payment.php');
    include_once(ROOT_PATH . 'include/lib_order.php');
    ...
    $url_ce="/mobile/flow.php?step=ok&order_id=".$order['order_sn']."&state=STATE";
    $order['pay_online']="<div style='width:90px;height:30px;'><a href='".$url_ce."' style='height:10px;margin-top:8px;padding:5px;background-color:#00D20D;color:#fff;'>微信支付</a></div>";
    $order['to_buyer'] = null;
    $smarty->assign('order',      $order);
    $smarty->assign('goods_list', $goods_list);
    $smarty->display('user_transaction3.dwt');
}
/*-------------------------------测试订单支付  end------------------------------------*/
/* 取消订单 */
...
elseif ($action == 'address_list')
{
    ...
    $i = 0;
    $sql = "select region_name from ecs_region where region_id = %s";
    foreach ($consignee_list AS $region_id => $consignee)
    {
    	...
    	$province_list[$region_id] = $province_list1[$i]= get_regions(1, $consignee['country']);
    	$city_list[$region_id]     = $city_list1[$i] = get_regions(2, $consignee['province']);
    	$district_list[$region_id] = $district_list1[$i] = get_regions(3, $consignee['city']);
    	
    	
    	$address_name = $address_name.$db->getOne(vsprintf($sql,array($consignee['province'])))." ";
    	$address_name = $address_name.$db->getOne(vsprintf($sql,array($consignee['city'])))." ";
    	$address_name = $address_name.$db->getOne(vsprintf($sql,array($consignee['district'])))." ";
    	//echo var_dump($province_list[$region_id]);
    	//echo var_dump($city_list[$region_id]);
    	//echo var_dump($district_list[$region_id]);
    	$consignee_list[$i]['mobile'] = substr($consignee['mobile'],0,3).'****'.substr($consignee['mobile'],7) ;
    	$consignee_list[$i]['address_name'] = $address_name;
    	//echo $address_name."<br/>";
    	$i = $i+1;
    }
    //echo var_dump($consignee_list);
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
        ...
        'country'    => isset($_POST['country'])   ? intval($_POST['country'])  : '86',
...
elseif ($action == 'async_collection_list'){
    ...
    if(is_array($collections)){
        foreach($collections as $vo){
            ...
            $t_price = (empty($vo['promote_price']))? $_LANG['shop_price'].'<span class="price">'.$vo['shop_price'].'</span>':$_LANG['promote_price'].'<span class="price">'.$vo['promote_price'].'</span>';

			
			$smarty->assign('cover_img', '<a href="'.$vo['url'].'"><img src="'.$config['site_url'].$img.'" /></a>');
			$smarty->assign('item_name', $vo['goods_name']);
			$smarty->assign('item_desc', $vo['goods_guide']);
			$smarty->assign('item_extra', $t_price);
			$smarty->assign('right_top_widget', '<a href="'.$vo['url'].'"><img src="themes/vip/images/category/cart.png"></a>');
			$smarty->assign('right_bottom_widget', '<a href="javascript:if (confirm(\''.$_LANG['remove_collection_confirm'].'\')) location.href=\'user.php?act=delete_collection&collection_id='.$vo['rec_id'].'\'" style="color:#1CA2E1"><img src="themes/vip/images/xin/icoshanchu.png"></a>');
			$smarty->assign('item_url', $vo['url']);
			$single_item = $smarty->fetch('library/single_item.lbi');
            $asyList[] = array(
                'collection' => $single_item
//		'<a href="'.$vo['url'].'"><table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table_no_border">
//            <tr>
//                <td style="width:100px;padding:1px;"><img src="'.$config['site_url'].$img.'" width="100%"/></td>
//                <td style="padding:1px;">'.$vo['goods_name'].'<br>'.$t_price.'</td>
//                <td align="right"  style="width:70px;padding:1px;"><a href="'.$vo['url'].'" style="color:#1CA2E1">'.$_LANG['add_to_cart'].'</a><br><br><a href="javascript:if (confirm(\''.$_LANG['remove_collection_confirm'].'\')) location.href=\'user.php?act=delete_collection&collection_id='.$vo['rec_id'].'\'" style="color:#1CA2E1">'.$_LANG['drop'].'</a></td>
//            </tr>
//          </table></a>'
            );
//            $asyList[] = array(
//            	'goods_img' => '<a target="_blank" href="goods.php?id=' . $vo['goods_id'] . '"><img border="0" src="'.$config['site_url'].$img.'" class="lazy" style="padding-left: 14px;width: 100px; padding-left: 5px;"></a>',
//            	'goods_name' => '<a class="fragment" target="_blank" href="goods.php?id=' . $vo['goods_id'] . '" style="color:#333"><span style="height:2rem;">'.$vo['goods_name'].'</span></a>',
//            	'price' => $t_price,
//            	'addToCart' => '<a href="'.$vo['url'].'"></a>',
//            	'remove_collection' => '<a href="javascript:openConfirmDiv(\''.$_LANG['remove_collection_confirm'].'\', \'location.href="user.php?act=delete_collection&collection_id='.$vo['rec_id'].'";\')"> </a>'
//            );
        }
    }
    echo json_encode($asyList);
...
elseif ($action == 'account_deposit')
{
    ...
    $smarty->assign('payment', get_online_payment_list(false,1));
...
elseif ($action == 'user_card')//绑定会员卡号
{
	if($_POST['bind'])
	{
	   ...
	   if($card_info)
	   {
		  ...
		  elseif($card_info['card_pass'] != $card_pass)
		  {
		      show_message("卡密错误！");
		  }
		  else
...
elseif ($action == 'bonus')
{
    ...
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page);
    $bonus = get_user_bouns_list($user_id, $record_count, $pager['start']);
...
elseif ($action == 'order_comments')//zmh 订单评论页面
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	include_once(ROOT_PATH . 'include/lib_payment.php');
	include_once(ROOT_PATH . 'include/lib_order.php');
...
elseif ($action == 'problem_lists')// 问题查询列表页面
{
   
    //组织sql语句,$cat_id是文章问题分类id
    $cat_id = 16;
    $sql = "SELECT article_id,title FROM " . $ecs->table('article') . " WHERE cat_id = '".$cat_id."' AND is_open = 1";
    $problem_list = $db->getAll($sql);

    $smarty->assign('action',$action);
    $smarty->assign('problem_list', $problem_list);
    $smarty->display('problem_lists.dwt');
}
