...
$logger = LoggerManager::getLogger('search.php');
...
else
{
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.click_count ,g.sales_count, g.market_price,g.sales_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.goods_guide, ".
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['zong_count'] = $row['sales_count'];
    ...
    if(isset($_REQUEST['ajax_more']) && $_REQUEST['ajax_more'] == '1'){
		foreach ($arr as $item){
			...
			$arr_more[] = array(
					'proImg-wrap' => '<a href="'.$item['url'].'" > <img src="'.$config['site_url'].$item['goods_thumb'].'" alt="'.$item['goods_name'].'"/>',
					'proTitle' => '<a href="'.$item['url'].'" >'.$item['goods_name'].'</a>',
					'proSubTitle' => '<a href="'.$item['url'].'" >'.$item['goods_guide'].'</a>',
					'proPrice' => empty($item['promote_price']) ? '<em>' . $item['shop_price'] . '</em>' . '<del>'.$item['market_price'].'</del>' : '<em>' . $item['promote_price'] . '</em>' . '<del>'.$item['market_price'].'</del>',
// 					'proService' => '<del>'.$item['market_price'].'</del>',
					'proSales' => '销量:<em>'.$item['sales_count'].'</em>',
