...
require_once(dirname(__FILE__) . '/include/lib_order.php');
require(ROOT_PATH . 'include/lib_weixintong.php');
/* 载入语言文件 */
require_once(ROOT_PATH . 'lang/' .$_CFG['lang']. '/user.php');
require_once (dirname(__FILE__) . '/api/weixin_api.php');
$weixin_api = new weixin_api();
$logger = LoggerManager::getLogger('distribute.php');
...
/* 显示页面的action列表 */
$ui_arr = array('register','userrank', 'login', 'profile','dianpu', 'act_dianpu', 'order_list', 'order_detail', 'order_tracking', 'address_list', 'act_edit_address', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','point','user_card','fenxiao1','myorder','myorder_detail','fenxiao2','fenxiao3','fenxiao4','fenxiao5','fenxiao');
/* 未登录处理 */
...
if (in_array($action, $ui_arr))
{
    ...
    $smarty->assign('page_title', "我的窝"); // 页面标题
...
if ($action == 'default')
{
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        */
        ...
        $all_sales_paied+=${sales.$i._paied}['all_sales'];		//佣金
        $sales_all_paied+=${sales.$i._paied}['order_amount'];	//订单金额

        /* 已收货佣金统计  */
        $sql="SELECT count(*) as order_num ,sum(sales".$i."_money)  as order_amount ,sum(goods_amount) as all_sales FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id in (".$up_uid_all.") and  pay_status=2 and   shipping_status  =2";
        ${sales.$i._over}=$db->getRow($sql);
        $all_sales_over+=${sales.$i._over}['all_sales'];	//佣金
        $sales_all_over+=${sales.$i._over}['order_amount'];	//订单金额
    }
    ...
    $setmoneyPersonal1 = round($order_info['order_amount'] * $affiliate['config']['level_money_personal']*0.01, 2);
    $tianxinarrPersonal['weifukuan']['setmoneyPersonal'] = $setmoneyPersonal1;
    */

    //已付款
    $sql="SELECT sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$_SESSION['user_id']." and  pay_status=2 and shipping_status <> 2";
    $order_info=$db->getRow($sql);
    $setmoneyPersonal2 = round($order_info['order_amount'] * $affiliate['config']['level_money_personal']*0.01, 2);
    $tianxinarrPersonal['yifukuan']['setmoneyPersonal'] = $setmoneyPersonal2;
    //已收货
    $sql="SELECT sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$_SESSION['user_id']." and  shipping_status =2";
    $order_info=$db->getRow($sql);
...
// 	$tianxin100all['order_amount']=$all_sales_notpay+$all_sales_paied+$all_sales_over;
	$tianxin100all['setmoney']=$sales_all_paied+$sales_all_over;	//累计奖励
	$tianxin100all['order_amount']=$all_sales_paied+$all_sales_over;//累计销售
	...
	$smarty->assign('tianxinarrPersonal',        $tianxinarrPersonal);
	...
	$ex_fenxiao_personal = $affiliate['config']['ex_fenxiao_personal'];
	$smarty->assign('ex_fenxiao_personal',       $ex_fenxiao_personal);
...
elseif($action == 'userrank'){
	/* 获取会员信息 */

	 $sql = "SELECT @rank:=@rank+1 as pm , A.*
     FROM
     (
      SELECT  t1.user_id,t2.user_name,sum(t1.goods_amount ) as sum_order
       FROM ". $GLOBALS['ecs']->table('order_info') ." t1
       		LEFT JOIN ". $GLOBALS['ecs']->table('users') . " t2 on t1.user_id=t2.user_id WHERE t1.order_status = '5' and t1.shipping_status = '2' GROUP BY t1.user_id ORDER BY sum_order DESC LIMIT 0,30
     ) A ,(SELECT @rank:=0) B ";
	$paihang_list = $GLOBALS['db']->getAll($sql);
	$smarty->assign('user_list', $paihang_list);
    $smarty->display('distribute.dwt');
}
...
elseif ($action == 'login')
{
    ...
    $smarty->assign('back_act', $back_act);
    $smarty->display('user_passport_tianxin100.dwt');
}
...
{
    $user_id = $_SESSION['user_id'];
    $logger->debug('bind = ' . $_POST['bind']);
    $logger->debug('bind2 = ' . $_POST['bind2']);
    if (!$_POST['bind']){
    	$mobile_phone = $db -> getOne("select mobile_phone from ".$ecs->table('users')." where user_id = $user_id");
    	if (empty($mobile_phone)){
    		$logger->debug('dwt_action = account_raply_psw');
    		$smarty->assign('action', 'account_raply_psw');
    		$smarty->assign('page_title', '绑定手机');
    	}/*else {
    		 if ($_POST['bind2']){
    			$password_account_replay_input = trim($_POST['password_account_replay']);
    			if (!empty($password_account_replay_input)){
    				$password_account_replay_input = md5($password_account_replay_input);
    				if ($password_account_raplay == $password_account_replay_input){
    					$logger->debug('dwt_action = account_raply');
    					$smarty->assign('action', 'account_raply');
    				}else {
    					show_message("密码错误， 请重试");
    				}
    			}else{
    			...
    			$logger->debug('dwt_action = account_raply_checkpsw');
    			$smarty->assign('action',   'account_raply_checkpsw');
    		} 
		...
		$limit_account_replay = $db -> getOne("SELECT limit_account_replay FROM wxch_config");
		$smarty->assign('limit_account_replay',   $limit_account_replay);
		$smarty->assign('sur_amount',   $sur_amount);
		$smarty->assign('_CFG',   $GLOBALS['_CFG']);
    	$smarty->display('user_transaction1.dwt');
    }else {
    	$password_account_replay1 = trim($_POST['password_account_replay1']);
    	$password_account_replay2 = trim($_POST['password_account_replay2']);
    	$wxh = trim($_POST['wxh']);
    	
    	$mobile_phone = isset($_REQUEST['mobile']) ? trim($_REQUEST['mobile']) : '';
    	$verifycode = isset($_REQUEST['verify_code']) ? trim($_REQUEST['verify_code']) : '';
    	if ($password_account_replay1 != $password_account_replay2){
    		show_message("两次输入的密码不一样");
    	}    	
    	...
    	if ($_CFG['ecsdxt_mobile_bind'] == '1')
    	{
    		...
    		$db->query($sql);
    		$sql = "UPDATE " . $ecs->table('users') . " SET mobile_phone='" . $mobile_phone ."',wxh = '".$wxh."' WHERE user_id=" . $user_id . "";
    		$mfy = $db->query($sql);
    		//发红包
    		if($mfy){
    			$wxid = $db -> getOne("select wxid from ".$ecs->table('users')." where user_id = $user_id");
    			
    			/* 取得订单应该发放的红包 */
    			$bonus_list = mobile_bonus(4);
    			/* 如果有红包，统计并发送 */
    			if ($bonus_list)
    			{
    				foreach ($bonus_list AS $bonus)
    				{
    					$sql = "update ecs_user_bonus set user_id = $user_id where bonus_id = ".$bonus["bonus_id"];
    					if ($GLOBALS['db']->query($sql)){
    						$weixin_api->send_custom_message($wxid,"你送到一个《".$bonus["type_name"]."》红包");
    					}
    				}
    			}
    	...
    	show_message("保存成功", '返回分销中心', 'distribute.php', 'info');

    	/* if (!empty($password_account_replay1))
    	{
    		$password_account_replay1 = md5($password_account_replay1);
    		$mfy = $db -> query("update ".$ecs->table('users')." set password_account_replay = '$password_account_replay1' where user_id = $user_id");
    		if ($mfy){
    			show_message("保存成功", '返回分销中心', 'distribute.php', 'info');
    		}else {
    			show_message("保存失败，请重试");
    		}
    	}else {
    		show_message("请输入密码");
    	} */
...
elseif ($action == 'account_log')
{
    ...
    //分页函数
    $pager = get_pager('distribute.php', array('act' => $action), $record_count, $page);
...
elseif ($action == 'act_account')
{
	...
	$limit_account_replay = $db -> getOne("SELECT limit_account_replay FROM wxch_config");
	if ($amount < $limit_account_replay){

		show_message("请输入大于提现额度的金额");
	}

	$tixian_fangshi = $_POST['tixian_fangshi'];

	if(empty($tixian_fangshi))
	{
		show_message("请选择提现方式");
	}
	else
	{
		if($tixian_fangshi==1) //银行卡提现
		{
			/*青山老农分销微分销优化提现流程 */
		...
		elseif($tixian_fangshi==2)//微信红包提现
		{
			$wx_num = $_POST['wx_num'];//微信号
			$user_note = $_POST['user_note'];//留言
			$tianxin100 = "微信号:【".$wx_num."】留言:【".$user_note."】";
		}
		elseif($tixian_fangshi==3)//支付宝提现
		{
			$zhifubao_num = $_POST['zhifubao_num'];//支付宝账号
			$zhifubao_name=$_POST['zhifubao_name'];//支付宝姓名
			$user_note=$_POST['user_note'];//留言
			$tianxin100="支付宝账号:【".$zhifubao_num."】支付宝姓名:【".$zhifubao_name."】留言:【".$user_note."】";
		}
	}
	/*青山老农分销微分销优化提现流程*/
    /* 变量初始化 */
...
elseif ($action == 'fenxiao')
{
    ...
    $sql = "SELECT user_id, user_name, '$rank' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,wxid ".
        " FROM " . $GLOBALS['ecs']->table('users') . " WHERE sales".$rank."_id =($auid)" .
        " ORDER by level, user_id";
    $user_info=$db->getAll($sql);

    foreach($user_info as $key=>$value){

        $sql="SELECT count(*) as order_num ,sum(sales".$rank."_money)   as order_amount ,sum(order_amount) as all_sales  FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and order_status not in (0,2,4,7) " ;
        $order_info=$db->getRow($sql);

        $user_info[$key]['order_num']=$order_info['order_num'];
        $user_info[$key]['order_amount']=$order_info['order_amount'];
        $user_info[$key]['setmoney']=$order_info['all_sales'];
	...
	$rank = str_replace('1',"金牌窝友",$rank);
	$rank = str_replace('2',"银牌窝友",$rank);
	$count=count($new_arr);
    ...
    if(!empty($affiliate['on']))
    {
        if(empty($separate_by))
        {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    ...
                    " WHERE o.user_id > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd and o.order_status not in (0,2,4,7)";
        }
        else
        {
            //推荐订单分成
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (o.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd and o.order_status not in (0,2,4,7)";
        }
    }
    else
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
                " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                " WHERE o.user_id > 0 AND o.is_separate > 0 $sqladd and o.order_status not in (0,2,4,7)";
    }
	...
	foreach($logdb  as $key=>$value){
		...
		//print_r($value);
			...
			if($value['is_separate']==1){
				$if_tianxin100=$value['user_id']==$user_id&&$value['suid']==$_SESSION['user_id'];
			}else{
				$if_tianxin100=$value['user_id']==$user_id;
			}
		if($if_tianxin100){
			$logdbnew[$key]=$value;
			...
			if(!empty($wxid)){
                ...
                $fc_n = 'sales'.$level.'_money';
				$setmoney = round($value[$fc_n], 2);
				$logdbnew[$key]['set_money']=$setmoney;
				$logdbnew[$key]['level_money']=round($setmoney/$value['fencheng'],2) * 100 . '%';
