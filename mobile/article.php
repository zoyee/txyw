...
$logger = LoggerManager::getLogger('article.php');
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
	...
	if(!empty($userid)){
		...
		//生成分享二维码
		if($affiliate['on']){
			$qr_url = $config['site_url']. 'mobile/' . "qrcode.php?";
			$qr_url .= "act=article&id=$article_id&user_id=" . $userid;
			$result = file_get_contents($qr_url);
		...
		if($u){
			$qr_url = $config['site_url']. 'mobile/' . "qrcode.php?";
			$qr_url .= "act=article&id=$article_id&user_id=" . $u;
			$result = file_get_contents($qr_url);
			$logger->debug('$result = ' . $result);
			$json = json_decode($result);
			if($json->errcode == 0){
	...
	if(!empty($u)){
		...
		$sql_two="SELECT * FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE ip = '$iipp'";	
		$ipinfo=$GLOBALS['db']->GetRow($sql_two);
		$jf_state = $db -> getOne("SELECT `autoload` FROM `wxch_point` WHERE `point_name` = 'share_arpoint'");
		if ($jf_state == 'yes'){
			if(empty($ipinfo)){
		
				$qd_jf = $db -> getOne("SELECT `point_value` FROM `wxch_point` WHERE `point_name` = 'share_arpoint'");
				log_account_change($u, 0, 0, 0, $qd_jf, "分享文章点击获取",3,$iipp);
			}
	/* 	$sql="SELECT count(*) FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE user_id = '$u' and change_type=3";	
		$count = $GLOBALS['db']->getOne($sql);
			if($count==1){
		
					log_account_change($u, 0, 0, 0, -$point, "分享文章点击获取",3,$iipp);
...
/*t ianx  in  1 00  开 发 转发文章页也可以加积分和形成上下级关系 */
