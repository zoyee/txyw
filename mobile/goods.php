...
$logger->debug('affiliate = ' . json_encode($affiliate));
...
// $logger->debug(json_encode($_SESSION));
/* 代码增加_start By  www.ecshop68.com */
$sql_attr_www_ecshop68_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE  ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
		...
		$tianxin100=price_format($tianxin100);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'allprice')
{
	...
	if($row)
	{
		...
		//file_put_contents('./77.txt',var_export($ret1,true));
...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_province_distribution'){
	//查询商品省份分布
	$sql = "select t.name, sum(t.value) as value from
			(select r.region_id, r.region_name as name, IF(g.goods_id is null, 0, 1)as value
			from ecs_region r
			left join ecs_suppliers s on s.province = r.region_id
			left join ecs_goods g on g.suppliers_id = s.suppliers_id and g.is_on_sale = 1
			where r.parent_id='86') t
			group by t.region_id, t.name";
	$data = $db->getAll($sql);
    ...
    else
    {
       ...
       $goods['goods_desc'] = str_replace('src="/images/', 'src="'.$config['site_url'].'images/', $goods['goods_desc']); //修复产品详情的图片 by wang
		...
		if($goods['suppliers_id']){
			$supplier = $db->getRow("SELECT a.*, p.region_name AS province_name, ct.region_name AS city_name, d.region_name AS district_name ".
		           " FROM " .$ecs->table('suppliers'). " as a ".
			...
			$check_apply = $db->getOne('select count(1) from ' . $ecs->table('suppliers_user') . " where suppliers_id='$suppliers_id' and user_id='$_SESSION[user_id]' and status=5");
			if($check_apply){
				$smarty->assign('is_agent', 1);
			}
		}
...
$smarty->assign('goods_n',$goods_n);
		...
		if(!empty($userid)){
			...
			$url_home=$config['site_url']."mobile/";
			//20141204新增分享返积分
...
$index_share_title = $db->getOne("SELECT value FROM ecs_touch_shop_config WHERE code = 'goods_share_title'");
$index_share_content = $db->getOne("SELECT value FROM ecs_touch_shop_config WHERE code = 'goods_share_content'");
$share_autoload = $db->getOne("select autoload from wxch_point where point_id = 12");
$smarty->assign('share_title', str_replace("#goodsName", $goods["goods_style_name"], str_replace("#userName", $info['nickname'] , $index_share_title)));
$smarty->assign('share_content', str_replace("#goodsName", $goods["goods_style_name"], str_replace("#userName", $info['nickname'] ,$index_share_content)));
$smarty->assign('autoload', $share_autoload=='yes'?'1':'');
