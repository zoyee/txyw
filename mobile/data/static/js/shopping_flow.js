...
function orderSelectedResponse(result)
{
  ...
  {	  
    ...
    if (result.payment != undefined)
    {
	  ...
	  //修改可用积分
	  var order_max_integral = jQuery("#order_max_integral").val();
	  jQuery("#order_max_integral_show").html(order_max_integral);
  }
  catch (ex) { }
}
...
function changeSurplus(val)
{
  ...
  var integral = jQuery("#ECS_INTEGRAL").val();
  Ajax.call('flow.php?step=change_surplus', 'surplus=' + val + "&integral=" + integral, changeSurplusResponse, 'GET', 'JSON');
}
...
{
	var integral = jQuery("#ECS_INTEGRAL").val();
  Ajax.call('flow.php?step=change_bonus', 'bonus=' + val + "&integral=" + integral, changeBonusResponse, 'GET', 'JSON');
}
...
function changeBonusResponse(obj)
{
  ...
  else
  {
	...
	document.getElementById('bonus_box').style.display="block";
    orderSelectedResponse(obj.content);
  }
}
...
function checkConsignee(frm)
{
  ...
  if(frm.elements['mobile'].value.length != 11 )
  {
	  err = true;
	  msg.push(mobile_invaild);
  }
