...
function T($template = '', $layer = '') {
	...
	if (defined ( '_ADDONS' ) && false === strpos ( $template, '://' )) {
		if (empty ( $template )) {
			$file_path = SITE_PATH . '/Addons/' . _ADDONS . '/View/default/' . _CONTROLLER . '/' . _ACTION . $temp_fix;
		} elseif (false === strpos ( $template, '/' )) {
			$file_path = SITE_PATH . '/Addons/' . _ADDONS . '/View/default/' . _CONTROLLER . '/' . $template . $temp_fix;
		} else {
			$file_path = SITE_PATH . '/Addons/' . _ADDONS . '/View/default/' . $template . $temp_fix;
		}
...
function import($class, $baseUrl = '', $ext = EXT) {
	...
	if (empty ( $baseUrl )) {
		...
		} elseif (in_array ( $class_strut [0], array (
				'Think',
				'Org',
				'Behavior',
				'Com',
				'Vendor' 
		) ) || is_dir ( LIB_PATH . $class_strut [0] )) {
			// 系统类库包和第三方类库包
			$baseUrl = LIB_PATH;
		} else { // 加载其他模块的类库
			$baseUrl = APP_PATH;
