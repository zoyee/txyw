...
class XydzpController extends AddonsController
{
    ...
    public function zjloglists()
    {
        ...
        if (false === $key) {
            array_push($fields, "{$fix}member.openid as openid");
        } else {
            $fields[$key] = "{$fix}member.openid as openid";
        }
		
		$key = array_search('truename', $fields);
        if (false === $key) {
            array_push($fields, "{$fix}member.truename as truename");
        } else {
            $fields[$key] = "{$fix}member.truename as truename";
        }
		
		$key = array_search('mobile', $fields);
        if (false === $key) {
            array_push($fields, "{$fix}member.mobile as mobile");
        } else {
            $fields[$key] = "{$fix}member.mobile as mobile";
        }
		
		
        //中奖列表
		$sqlqu = M('xydzp_log')
					->join('left join ' . $fix . 'member on ' . $fix . 'xydzp_log.uid=' . $fix . 'member.openid')
					->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_log.xydzp_option_id=' . $fix . 'xydzp_option.id')
					->field(empty($fields) ? true : $fields)
					->where(array('xydzp_id' => $xydzp_id,$fix . 'member.token' => get_token() ));
		...
		if($isend){
			...
			if($url == ""){
				...
				$zjuserlist = M('xydzp_log')
					->join('left join ' . $fix . 'member on ' . $fix . 'xydzp_log.uid=' . $fix . 'member.openid')
					->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_log.xydzp_option_id=' . $fix . 'xydzp_option.id')
					->field($fix . "xydzp_log.id," . $fix . "member.truename," . $fix . "xydzp_option.title,zjdate")
					->where(array('xydzp_id' => $xydzp_id,$fix . 'member.token' => $token ))
					->order("zjdate desc")
					->limit(10)
					->select();
	...
	function zjinfo(){
        ...
        $zjuserlist = M('xydzp_log')
				...
				->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_log.xydzp_option_id=' . $fix . 'xydzp_option.id')
				->field($fix . "xydzp_log.id," . $fix . "member.truename," . $fix . "xydzp_option.title,zjdate")
				->where(array('xydzp_id' => $xydzp_id,$fix . 'member.token' => $token, $fix . 'xydzp_log.uid'=>$openid ))
				->order("zjdate desc")				
				->select();
    ...
    function join()
    {
		...
		if($this->_is_starttime($xydzp_id)){
			if($this->_is_overtime($xydzp_id)){
				...
				$fix      = C("DB_PREFIX");
				//查询奖品列表		
				$jplist   = M('xydzp_jplist')->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_jplist.xydzp_option_id=' . $fix . 'xydzp_option.id')->field($fix . "xydzp_option.title,gailv,xydzp_id,xydzp_option_id," . $fix . "xydzp_option.num as kcnum," . $fix . "xydzp_option.miaoshu")->where("xydzp_id=$xydzp_id")->order('type asc,gailv asc')->select();
				//按奖品等级计算概率
    ...
    */
    ...
    {
        $the_vote = M("xydzp")->where("id=$xydzp_id")->find();
        $deadline = $the_vote['start_date'] + 86400;
        if ($deadline >= time()) {
            return true;
        }
        return false;
