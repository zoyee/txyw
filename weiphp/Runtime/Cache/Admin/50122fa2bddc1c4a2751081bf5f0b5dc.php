...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/weiphp/Public/Admin/css/<?php echo (C("COLOR_STYLE")); ?>.css?v=<?php echo SITE_VERSION;?>" media="all">
     <!--[if lt IE 9]>
...
<!--         <img style="height:49px;" src="/weiphp3.0/Public/Home/images/logo.png"> -->
    ...
    <div class="cf">
		...
		<div class="search-form fr cf">
            <div class="sleft">
                <select name="group">
                        <option value="<?php echo U('Admin/Addons/index',array('cate_id'=>0));?>" <?php if(($$cate_id) == "0"): ?>selected<?php endif; ?> >全部插件</option>
                        <option value="<?php echo U('Admin/Addons/index',array('cate_id'=>'-1'));?>" <?php if(($$cate_id) == "-1"): ?>selected<?php endif; ?> >无分类插件</option>
                    <?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Admin/Addons/index',array('cate_id'=>$vo['id']));?>" <?php if(($vo['id']) == $cate_id): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
            <div class="sleft">
 ...
 $class = get_addon_class($vo['name']); if(!class_exists($class)){ $has_config = 0; }else{ $addon = new $class(); $has_config = count($addon->getConfig()); } ?>
    ...
    <script type="text/javascript" src="/weiphp/Public/Admin/js/common.js?v=<?php echo SITE_VERSION;?>"></script>
    <script type="text/javascript">
        +function(){
	...
	<script src="/weiphp/Public/static/thinkbox/jquery.thinkbox.js?v=<?php echo SITE_VERSION;?>"></script>
