...
elseif ($_REQUEST['step'] == 'add_to_cart_showDiv')
{
    ...
    /* 检查：是否超出限购数量 */
    $sql="SELECT buymax FROM " . $GLOBALS['ecs']->table('goods') .
            " WHERE buymax_start_date < ".time()
            ." AND ".time()." < buymax_end_date 
                AND buymax > 0 
                AND goods_id = ".$goods->goods_id;
        $res = $GLOBALS['db']->getOne($sql);
    if($res > 0){
        //获得当前商品在购物车中数量
        $sql="SELECT goods_number FROM ". $GLOBALS['ecs']->table('cart') ."WHERE goods_id = "
                .$goods->goods_id." AND session_id = '".SESS_ID."'";
        $cart_goods_numbers = $GLOBALS['db']->getOne($sql);
        if($res < $goods->number + $cart_goods_numbers){
            $result['error']   = 1;
            $result['message'] = '购物车已有'.$cart_goods_numbers.'件该商品，总共超出限购数量';
            die($json->encode($result));
        }
    }
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
	    ...
	    if($affiliate['on']){
			...
			if($json->errcode == 0){
				$db->query("insert into " . $GLOBALS['ecs']->table('qr_event') . " (uuid, type, status, dateline)values('$uuid', 1, 0, '$dateline')");
	    ...
	    $smarty->assign('back_act', "flow.php?step=consignee");
