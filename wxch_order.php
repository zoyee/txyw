...
$logger = LoggerManager::getLogger('wxch_order.php');
$logger->debug('$wxch_user_id = ' . $wxch_user_id);
$time = time();
...
$nickname = "";
$logger->debug('$order = ' . json_encode($order));
if($wxch_user_id > 0){
// 	$access_token = access_token($GLOBALS['db']);
// 	$url = 'https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$access_token;
	$query_sql = "SELECT * FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$wxch_user_id'";
...
// 		$pay_status = '支付状态：已付款';
	...
	$logger->debug('$pay_status = ' . $pay_status);
// 	$wxch_address = "\r\n收件地址：".$orders['address'];
...
// 	$w_title = $cfg_order['title'];
	...
	$logger->debug('order_amount = ' . $orders['order_amount']);
...
// 	}
	...
	$weixin_api->send_template_msg("pay", array(
			...
			"goods_name" => $shopinfo,
			"tatol_amount" => $orders['money_paid'],
	...
	$gwcj_url = "http://shop.byhill.com/wechat/dzp/index.php?pid=3&wxid=".$wxid;
	if($prize['price'] <=$orders['money_paid'] && $prize['is_show'] == 1){
