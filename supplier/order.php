...
$logger = LoggerManager::getLogger('order.php');
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    if(in_array($admin_info['user_name'], explode(',', SUPPLIER_USERS))){
    	 $smarty->assign('supper_admin', 1);
    }
    ...
    if($_REQUEST["isDwonload"]){
    	...
    	//表格宽度
    	$PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(20);//订单编号
    	$PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(20);//收货人
    	$PHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(40);//收货地区
    	$PHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(40);//收货地址
    	$PHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(18);//收货人电话
    	$PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);//下单时间
    	$PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(20);//付款时间
    	$PHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(20);//买家留言
    	$PHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(20);//商品编号
    	$PHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(20);//商品名称
    	$PHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(20);//商品属性
    	$PHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(10);//应发货
    	$PHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(10);//已发货
    	$PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(15);//发货状态
    	//$PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(20);//快递公司
    	$PHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth(20);//快递单号
    	$PHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(20);//供货价
    	$PHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth(20);//供应商
    	
    	//表格标题
    	$PHPExcel->getActiveSheet()->setCellValue('A1', '订单编码');
    	$PHPExcel->getActiveSheet()->setCellValue('B1', '收货人');
    	$PHPExcel->getActiveSheet()->setCellValue('C1', '收货地区');
    	$PHPExcel->getActiveSheet()->setCellValue('D1', '收货地址');
    	$PHPExcel->getActiveSheet()->setCellValue('E1', '收货人电话');
    	$PHPExcel->getActiveSheet()->setCellValue('F1', '下单时间');
    	$PHPExcel->getActiveSheet()->setCellValue('G1', '付款时间');
    	$PHPExcel->getActiveSheet()->setCellValue('H1', '买家留言');
    	$PHPExcel->getActiveSheet()->setCellValue('I1', '商品编号');
    	$PHPExcel->getActiveSheet()->setCellValue('J1', '商品名称');
    	$PHPExcel->getActiveSheet()->setCellValue('K1', '商品属性');
    	$PHPExcel->getActiveSheet()->setCellValue('L1', '应发货');
    	$PHPExcel->getActiveSheet()->setCellValue('M1', '已发货');
    	$PHPExcel->getActiveSheet()->setCellValue('N1', '发货状态');
    	//$PHPExcel->getActiveSheet()->setCellValue('N1', '快递公司');
    	$PHPExcel->getActiveSheet()->setCellValue('O1', '供货价');
    	$PHPExcel->getActiveSheet()->setCellValue('P1', '供应商');
    	$PHPExcel->getActiveSheet()->setCellValue('Q1', '快递单号');
    	$hang = 1;
	    ...
	    /* 文件标题 */
	    //echo ecs_iconv(EC_CHARSET, 'utf-8', "订单信息") . ",\n";
	    
	    /* 商品名称,订单号,商品数量,销售价格,销售日期 */
	    /* echo ecs_iconv(EC_CHARSET, 'utf-8', '订单编码') . ",";	    
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '收货人') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '收货地址') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '收货人电话') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '下单时间') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '付款时间') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '买家留言') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '商品编号') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '商品名称') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '商品属性') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '应发货') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '已发货') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '发货状态') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '快递公司') . ",";
	    echo ecs_iconv(EC_CHARSET, 'utf-8', '快递单号') . ",";
	    echo "\n"; */
	    
	    $dqSQL = 'select region_name from ' .$GLOBALS['ecs']->table('region').' where region_id = \'%s\'';
	    
	    
	    foreach ($order_list['orders'] AS $key => $value)
	    {
	    	
	    	$provinceSQL = vsprintf($dqSQL,array($value['province']));
	    	$citySQL = vsprintf($dqSQL,array($value['city']));	    	   	
	    	$districSQL = vsprintf($dqSQL,array($value['district']));
	    	$province = $GLOBALS['db']->getOne($provinceSQL);
	    	$city = $GLOBALS['db']->getOne($citySQL);
	    	$distric = $value['district'] ?$GLOBALS['db']->getOne($districSQL):"";
	    	
	    	$order_sn = '`' . $value['order_sn'];
	    	$consignee = $value['consignee'];
	    	$addressDQ = $province.' '.$city.' '.$distric;
	    	$address = $value['address'];
	    	$mobile = '`'.$value['mobile'];
	    	$add_time = local_date($GLOBALS['_CFG']['time_format'], $value['add_time']);
	    	$pay_time = local_date($GLOBALS['_CFG']['time_format'], $value['pay_time']);
	    	$postscript = $value['postscript'];
	    	$suppliers_name = $value['suppliers_name'];
	    	
	    	/* echo ecs_iconv(EC_CHARSET, 'utf-8', '`' . $value['order_sn']) . ",";	    	 
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', $value['consignee']) . ",";
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', $province.' '.$city.' '.$distric.' '.$value['address']) . ",";
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', '`'.$value['mobile']) . ",";
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', local_date($GLOBALS['_CFG']['time_format'], $value['add_time'])) . ",";
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', local_date($GLOBALS['_CFG']['time_format'], $value['pay_time'])) . ",";
	    	echo ecs_iconv(EC_CHARSET, 'utf-8', $value['postscript']) . ","; */
	    	
	    	$num = 0;
	    	
	    	/* $sql = "SELECT o.goods_sn,o.goods_name,IFNULL(o.goods_attr, '') goods_attr,o.goods_number,o.send_number,g.supply_price " .
	    			"FROM " . $ecs->table('order_goods') . " AS o " .
	    			
	    			"LEFT JOIN  " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id  " .
	    			"WHERE o.order_id = ".$value['order_id']." and g.suppliers_id=".$value['suppliers_id'];
	    	$res = $db->getAll($sql);
	    	$shuliang   = 0;
	    	$chanpin    = $hang;
	    	foreach($res as $key => $value){ */
	    		/*if($num == 0){
	    			...
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $value['goods_attr']) . ",";	    			
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $value['goods_number']) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $value['send_number']) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $value['send_number']==$value['goods_number']?'已发货':'未发货') . ","; 
	    		}else{*/
	    			/* echo ecs_iconv(EC_CHARSET, 'utf-8', $order_sn) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $consignee) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $address) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $mobile) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $add_time) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $pay_time) . ",";
	    			echo ecs_iconv(EC_CHARSET, 'utf-8', $postscript) . ",";
	    	...
	    	//$chanpin = 0;
	    		...
	    		//张茂华修改
	    	$hang++;
	    		$PHPExcel->getActiveSheet()->setCellValue('A' . $hang, $order_sn);//加个空格，防止时间戳被转换
	    		$PHPExcel->getActiveSheet()->setCellValue('B' . $hang, $consignee);
	    		$PHPExcel->getActiveSheet()->setCellValue('C' . $hang, $addressDQ);
	    		$PHPExcel->getActiveSheet()->setCellValue('D' . $hang, $address);
	    		$PHPExcel->getActiveSheet()->setCellValue('E' . $hang, $mobile);
	    		$PHPExcel->getActiveSheet()->setCellValue('F' . $hang, $add_time);
	    		$PHPExcel->getActiveSheet()->setCellValue('G' . $hang, $pay_time);
	    		$PHPExcel->getActiveSheet()->setCellValue('H' . $hang, $postscript);
	    		$PHPExcel->getActiveSheet()->setCellValue('I' . $hang, $value['goods_sn']);
	    		$PHPExcel->getActiveSheet()->setCellValue('J' . $hang, $value['goods_name']);
	    		$PHPExcel->getActiveSheet()->setCellValue('K' . $hang, $value['goods_attr']);
	    		$PHPExcel->getActiveSheet()->setCellValue('L' . $hang, $value['goods_number']);
	    		$PHPExcel->getActiveSheet()->setCellValue('M' . $hang,$value['send_number']);
	    		$PHPExcel->getActiveSheet()->setCellValue('N' . $hang, $value['send_number']==$value['goods_number']?'已发货':'未发货');
	    		 
	    		//$PHPExcel->getActiveSheet()->setCellValue('N' . $hang, '');
	    		$PHPExcel->getActiveSheet()->setCellValue('O' . $hang, $value['supply_price']);
	    		$PHPExcel->getActiveSheet()->setCellValue('P' . $hang, $suppliers_name);
	    		$PHPExcel->getActiveSheet()->setCellValue('Q' . $hang, '');
	    		
	    	/* }
	    	$hang = $hang + $shuliang; */
	    }
	    //设置单元格边框
	    $PHPExcel->getActiveSheet()->getStyle('A1:Q'.$hang)->applyFromArray($styleArray);
	    //设置自动换行
	    $PHPExcel->getActiveSheet()->getStyle('A2:Q'.$hang)->getAlignment()->setWrapText(true);
	    //设置字体大小
	    $PHPExcel->getActiveSheet()->getStyle('A2:Q'.$hang)->getFont()->setSize(12);
	    //垂直居中
	    $PHPExcel->getActiveSheet()->getStyle('A1:Q'.$hang)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	    //水平居中
	    $PHPExcel->getActiveSheet()->getStyle('A1:Q'.$hang)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
//-- ajax查询订单的快递信息
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'get_kuaidi'){
	$order_id = $_REQUEST['order_id'];
	$admin_info = admin_info();
//	$suppliers_id = $admin_info['suppliers_id'];
	$suppliers_id = $_REQUEST['suppliers_id'];
	
	$invoice_str = $db->getOne("select invoice_no from " . $ecs->table('order_info') . " where order_id='$order_id'");
	$kuaidi = array(
		'shipping' => '',
		'invoice' => '',
		'supplier' => $suppliers_id
	);
	if(strstr($invoice_str, '}')){
		//json快递号
		$json_obj = json_decode($invoice_str);
		$okd = $json_obj->kd;
		if(is_array($okd)){
			foreach($okd as $i => $vo){
				if($vo->supplier == $suppliers_id){
					$kuaidi = $vo;
					break;
				}elseif(empty($vo->supplier) && empty($suppliers_id)){
					$kuaidi = $vo;
					break;
				}
			}
	...
	echo json_encode($kuaidi);
}
/*------------------------------------------------------ */
//-- ajax一键发货
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'delivery_order'){
	if(!admin_priv('order_ss_edit', '' , false)){
    	exit (json_encode(array('error' => 1, 'message' => '你没有权限进行此操作')));
    }
	$order_id = $_REQUEST['order_id'];
	$kuaidi_com = $_REQUEST['kuaidi_com'];
	$invoice_no = $_REQUEST['invoice_no'];
//	$logger->debug('$order_id = ' . $order_id);
//	$logger->debug('$kuaidi_com = ' . $kuaidi_com);
//	$logger->debug('$invoice_no = ' . $invoice_no);

	
	//echo  '$order_id = ' . $order_id.":".'$kuaidi_com = ' . $kuaidi_com.":".'$invoice_no = ' . $invoice_no;
	$admin_info = admin_info();
	$suppliers_id = $_REQUEST['suppliers_id'];
	if($suppliers_id == 0) $suppliers_id = '';
	
	$shipping_map = $db->getAll("select shipping_id, shipping_com from ecs_touch_shipping");
	$logger->debug(json_encode($shipping_map));
	
	$old_data = $db->getRow("select shipping_id, invoice_no, order_sn, order_status, pay_status from " . $ecs->table('order_info') . " where order_id='$order_id'");
	$shipping_com = "";
	foreach($shipping_map as $k => $map){
		if($old_data['shipping_id'] == $map['shipping_id']){
			$shipping_com = $map['shipping_com'];
		}
	}
	...
	if(empty($old_data['invoice_no'])){
		array_push($kd, $new);
	}elseif(strstr($old_data['invoice_no'], '}')){
		//json快递号
		$json_obj = json_decode($old_data['invoice_no']);
		$okd = $json_obj->kd;
		$add = false;
		foreach($okd as $i => $vo){
			if(!$add && $vo->supplier == $suppliers_id){
				array_push($kd, $new);
				$add = true;
			}else{
				array_push($kd, $vo);
		...
		if(!$add) array_push($kd, $new);
	}elseif(strstr($old_data['invoice_no'], ',')){
		//逗号分隔的多条快递号
		$old_invoices = split(',', $old_data['invoice_no']);
		$eq = false;
		foreach($old_invoices as $i => $vo){
			if($invoice_no == $old_data['invoice_no']){
				array_push($kd, $new);
				$eq = true;
			}else{
				array_push($kd, array(
					'shipping' => $shipping_com,
					'invoice' => $vo,
					'supplier' => ''
				));
			}
		}
		if(!$eq) array_push($kd, $new);
	}else{
		//单个快递号
		if($invoice_no == $old_data['invoice_no']){
			array_push($kd, $new);
	...
	$invoice_str = json_encode(array('kd' => $kd));
	$db->query("update " . $ecs->table('order_info') . " o, " . $ecs->table('order_goods') . " og, " . $ecs->table('goods') . " g " .
			"set o.invoice_no='$invoice_str', og.send_number = og.goods_number " . 
			"where o.order_id = og.order_id and og.goods_id = g.goods_id and g.suppliers_id='$suppliers_id' and o.order_id='$order_id'");
	$shipping_status = SS_SHIPPED_PART;
	if(get_order_finish($order_id)){
		$shipping_status = SS_SHIPPED;
	}
	$db->query("update " . $ecs->table('order_info') . " o set o.shipping_status='$shipping_status' where o.order_id='$order_id'");
	admin_log("订单".$old_data['order_sn']."=>".$kuaidi_com.'('.$invoice_no.')', 'setup', 'shipping');
	order_action($old_data['order_sn'], $old_data['order_status'], $shipping_status, $old_data['pay_status'], '供应商发货', $admin_info['admin_name']);
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    if(empty($suppliers_id)){
    	$sql .= " and (g.suppliers_id=0 or g.suppliers_id is null)";
    }else{
    	$sql .= " and g.suppliers_id='$suppliers_id'";
    }
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
...
elseif($_REQUEST['act'] == 'ImportExcel')
{
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		if(empty($admin_info['suppliers_id'])){
			$where = "WHERE 1=1";
		}else{
			$where = "WHERE g.suppliers_id='$admin_info[suppliers_id]'";
		}
        ...
        if ($filter['to_buyer'])
        {
            $where .= " AND o.to_buyer like '%$filter[to_buyer]%'";
        }
...
//                break;
            ...
            case CS_AWAIT_SHIP :
//                $where .= order_query_sql('await_ship');
				$where .= " and o.order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) . 
							" and o.pay_status = " . PS_PAYED .
							" and og.goods_number <> og.send_number";
                break;
...
//                break;
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(distinct o.order_id, IFNULL(g.suppliers_id,0), g.goods_sn, g.goods_name, og.goods_number, og.goods_attr) FROM " . $GLOBALS['ecs']->table('order_info') . " o inner join ". 
            		$GLOBALS['ecs']->table('order_goods') . " og on o.order_id = og.order_id inner join " . 
            		$GLOBALS['ecs']->table('goods') . " g on og.goods_id = g.goods_id LEFT join " .
                   $GLOBALS['ecs']->table('users') . " u on o.user_id = u.user_id " . $where;
        }
        else
        {
            $sql = "SELECT COUNT(distinct o.order_id, IFNULL(g.suppliers_id,0), g.goods_sn, g.goods_name, og.goods_number, og.goods_attr) FROM " . $GLOBALS['ecs']->table('order_info') . " o inner join ". 
            		$GLOBALS['ecs']->table('order_goods') . " og on o.order_id = og.order_id inner join " . 
            		$GLOBALS['ecs']->table('goods') . " g on og.goods_id = g.goods_id " . $where;
        }
        ...
        /* 查询 */
        $sql = "SELECT distinct o.order_id, o.order_sn, o.add_time,o.pay_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid,IFNULL(g.suppliers_id,0) as suppliers_id,o.postscript," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, o.mobile, o.province, o.city, o.district, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, o.invoice_no, g.goods_sn, g.goods_name, og.goods_number,og.send_number,g.supply_price, IFNULL(og.goods_attr, '') goods_attr,g.goods_thumb, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer, IFNULL(s.suppliers_name, '') as suppliers_name".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ".
				" INNER JOIN " .$GLOBALS['ecs']->table('order_goods'). " AS og ON og.order_id=o.order_id ".//acai2046 增加判断选项
				" INNER JOIN " .$GLOBALS['ecs']->table('goods'). " AS g ON g.goods_id=og.goods_id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('suppliers'). " AS s ON s.suppliers_id=g.suppliers_id ".
				$where .
	...
	//acai2046 供应商订单
//	$sql = "select * from (".$sql.") tt,ecs_suppliers es where es.suppliers_name='".$_SESSION['admin_name']."' and tt.suppliers_id=es.suppliers_id";
	//echo $sql;
    ...
    foreach ($row AS $key => $value)
    {
        ...
        //处理订单发货状态，校验快递号中有没有对应供应商的快递号
        $invoice_str = $row[$key]['invoice_no'];
        $fahuo = 0;
        ...
        if(strstr($invoice_str, '}')){
			...
			if(is_array($okd)){
				foreach($okd as $i => $vo){
					if($vo->supplier == $row[$key]['suppliers_id']){
						$fahuo = 1;
						$row[$key]['kdh'] = $vo->invoice;
						break;
					}elseif(empty($vo->supplier) && empty($row[$key]['suppliers_id'])){
						$fahuo = 1;
						$row[$key]['kdh'] = $vo->invoice;
						break;
					}else{
        ...
        $sql = "select region_name from ".$GLOBALS['ecs']->table('region') . " where region_id in('".$value['province']."','". $value['city']."','".$value['district']."')";
        $regions = $GLOBALS['db']->getAll($sql);
        $region_name = "";
        foreach($regions as $vo){
        	$region_name .= $vo['region_name'];
        }
        $row[$key]['region_name'] = $region_name;
    }
