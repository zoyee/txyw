...
//-- 保存商品详情
/*------------------------------------------------------ */
if($_REQUEST['act'] == 'save_goods_desc'){
	...
	make_json_result("保存成功");
}
/*------------------------------------------------------ */
//-- 删除商品轮播相册图片
/*------------------------------------------------------ */
elseif($_REQUEST['act'] == 'remove_gallery_img'){
	$goods_id = $_REQUEST['goods_id'];
	...
	//删除图片文件
	$gallery = $db->getRow("select * from " . $ecs->table('goods_gallery') . " where img_id='$img_id' and goods_id='$goods_id'");
	if($gallery){
		if($gallery['img_url']) @unlink(ROOT_PATH . $gallery['img_url']);
		if($gallery['thumb_url']) @unlink(ROOT_PATH . $gallery['thumb_url']);
		if($gallery['img_original']) @unlink(ROOT_PATH . $gallery['img_original']);
		
		$db->query("delete from " . $ecs->table('goods_gallery') . " where img_id='$img_id' and goods_id='$goods_id'");
		make_json_result("操作成功");
	}else{
		make_json_error('对应轮播图片不存在');
	}
...
elseif ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $goods_list = goods_list1($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1);
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        'goods_list1.htm' : (($_REQUEST['act'] == 'trash') ? 'goods_trash.htm' : 'group_list.htm');
...
//     }
    ...
    $suppliers_id = isset($_POST['suppliers_id']) ? intval($_POST['suppliers_id']) : '0';
		...
		//dqy add start 2011-7-12
		//$reason  = $_POST['reason'];
//		$is_pass =  $_POST['is_pass'];
		//dqy add end 2011-7-12
    ...
    //图片处理
    if(!empty($_POST['upload_goods_image'])){
        $original_img = $_POST['goods_image'];
        //缩略图路径
        $img_url_image = "../images/".date('Ym')."/goods_img/";
        $img_url_thumb = "../images/".date('Ym')."/thumb_img/";
        
        
        if($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0){
            $goods_img = $image->make_thumb('../'.$original_img,$GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height'],$img_url_image);
        }
        if($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0){
            $goods_thumb = $image->make_thumb('../'.$original_img,$GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height'],$img_url_thumb);
        }
        $goods_img = substr_replace ($goods_img,'',0,3);
        $goods_thumb = substr_replace ($goods_thumb,'',0,3);
...
//     $goods_thumb = (empty($goods_thumb) && isset($_POST['goods_image']))? $goods_img : $goods_thumb;

    //dqy add start 2011-7-13
    /* 入库 */
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, supply_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id , is_pass , goods_guide)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$_POST[supply_price]', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id' , '1' , '$_POST[goods_guide]')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral , is_pass , goods_guide)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$_POST[supply_price]', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','1' , '$_POST[goods_guide]')";
        }
				//dqy add end 2011-7-13
				
        
        //添加产品相册图片
//         $sql = "INSERT INTO " . $ecs->table('goods_gallery') . "  VALUES (null, '$_POST[goods_id]')";
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
...
//                "goods_desc = '$_POST[goods_desc]', " .
    ...
    if($is_insert){
        ...
        if(!empty($_POST['upload_goods_image'])){
            ...
            if($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0){
                $goods_img_f = $image->make_thumb('../'.$original_img,$GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height'],$img_url_image);
            }
            if($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0){
                $goods_thumb_f = $image->make_thumb('../'.$original_img,$GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height'],$img_url_thumb);
            }
            $original_img_f = '';
            $pos        = strpos(basename($original_img), '.');
            $original_img_f = dirname($original_img) . '/' . $image->random_filename() .  substr(basename($original_img), $pos);
            if (!copy('../' . $original_img, '../' . $original_img_f))
            {
                sys_msg('fail to copy file: ' . realpath('../' . $img), 1, array(), false);
            }
            $goods_img_f = substr_replace ($goods_img_f,'',0,3);
            $goods_thumb_f = substr_replace ($goods_thumb_f,'',0,3);
            

            $sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                "VALUES ('$goods_id', '$goods_img_f', '', '$goods_thumb_f', '$original_img_f')";
            $db->query($sql);
...
//     clear_cache_files();
    ...
    {
        $link[0] = array('href' => '/supplier/goods.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }else{
        if($_POST['goods_id'] > 0){
            $link[0] = array('href' => '/supplier/goods.php?act=edit&goods_id=' . $_POST['goods_id'], 'text' => $_LANG['product']);
        }else{
            $link[0] = array('href' => '/supplier/goods.php?act=edit&goods_id=' . $goods_id.'&extension_code=' , 'text' => $_LANG['product']);
        }
    }
    if ($code == 'virtual_card')
    {
    ...
    if ($is_insert)
    {
        ...
        $link[2] = array('href' => '/supplier/goods.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['edit_goods_ok']);
    }
...
//         krsort($link);
// //         $link = array_combine($key_array, $link);var_dump($link);
//     }
// var_dump($link);exit;
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $goods_list = goods_list1($is_delete, ($code=='') ? 1 : 0);
    ...
    $tpl = $is_delete ? 'goods_trash.htm' : 'goods_list1.htm';
