...
require(ROOT_PATH . 'languages/' .$_CFG['lang']. '/supplier/common.php');
require(ROOT_PATH . 'languages/' .$_CFG['lang']. '/supplier/log_action.php');

if (file_exists(ROOT_PATH . 'languages/' . $_CFG['lang'] . '/supplier/' . basename(PHP_SELF)))
{
    include(ROOT_PATH . 'languages/' . $_CFG['lang'] . '/supplier/' . basename(PHP_SELF));
}
...
$_CFG['site_url'] = $config['site_url'] . 'mobile/';
$_CFG['web_site_url'] = $config['site_url'];
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
			  ...
			  //$sql = "select * from ". $ecs->table('admin_user'). " where suppliers_id ='". intval($_COOKIE['ECSCP']['admin_id']) ."' ";
        ...
        else
        {
            ...
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['admin_pass'])
            {
                ...
                $db->query('UPDATE ' . $ecs->table('admin_user') .
							...
							// $db->query("UPDATE " .$ecs->table('admin_user').
//                 " SET last_login='" . gmtime() . "', last_ip='" . real_ip() . "'".
//                 " WHERE suppliers_id='$_SESSION[admin_id]'");
            }
            else
            {
                setcookie($_COOKIE['ECSCP']['admin_id'],   '', 1);
