 ...
 * @return  void
 */
function set_admin_session($suppliers_id, $suppliers_name, $action_list, $last_time)
{
    $_SESSION['admin_id']    = $suppliers_id;
    $_SESSION['admin_name']  = $suppliers_name;
    $_SESSION['action_list'] = $action_list;
    $_SESSION['last_check']  = $last_time; // 用于保存最后一次检查订单的时间
}

//设置供货商的session内容
function set_suppliers_session($suppliers_id,$suppliers_name,$action_list, $last_time)
{
	$_SESSION['suppliers_id'] = $suppliers_id;
	$_SESSION['suppliers_name'] = $suppliers_name;
	$_SESSION['action_list'] = $action_list;
...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    {
        $action_list = SUPPLIER_ACTION_LIST;
		//return true;
    }else{
