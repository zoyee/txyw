...
if ($_REQUEST['act'] == '')
{
	...
	foreach ($modules AS $key => $val)
	{
		...
		if (is_array($val))
		{
			foreach ($val AS $k => $v)
			{
				...
				//$logger->warn("菜单：".$k."[".$_LANG[$k]."]:".$v);
				//var_dump("菜单".$k.":".$_LANG[$k].":".$v);
	...
	//查询供应商信息
	$supplers_info = $db->getRow("select s.* from ecs_suppliers s
			inner join ecs_admin_user au on s.suppliers_id = au.suppliers_id and au.user_id='$_SESSION[admin_id]'");
	if($supplers_info){
		$smarty->assign('supplers_info', $supplers_info);		
	}
...
elseif ($_REQUEST['act'] == 'check_order')
{
    ...
    $supplers_id = $admin_info['supplers_id'];

    /* 新订单 */
    $sql = 'select count(1) from (SELECT distinct(o.order_id) FROM ' . $ecs->table('order_info').
    		" o, " . $ecs->table('order_goods') . " og, " . $ecs->table('goods') . 
			" g WHERE o.order_id=og.order_id and og.goods_id = g.goods_id and g.suppliers_id='$supplers_id' and o.add_time >= '$_SESSION[last_check]') t";
    $arr['new_orders'] = $db->getOne($sql);

    /* 新付款的订单 */
    $sql = 'select count(1) from (SELECT distinct(o.order_id) FROM ' . $ecs->table('order_info').
    		" o, " . $ecs->table('order_goods') . " og, " . $ecs->table('goods') . 
			" g WHERE o.order_id=og.order_id and og.goods_id = g.goods_id and g.suppliers_id='$supplers_id' and o.pay_time >= '$_SESSION[last_check]') t";
