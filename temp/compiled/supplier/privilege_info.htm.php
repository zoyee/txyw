...
function upload_img(keyword) {
	...
	$.ajaxFileUpload ({
		url:'privilege.php?act=upload_head_image&fname=upload_image_' + keyword,
		secureuri:false,
		...
		success: function (json, status) {
			if(json.error == 0) {
				$("#head_img").val(json.content);
				if($("#img_" + keyword).length >0){
					$("#img_" + keyword).attr("src", "../" + json.content);
				}else{
					$("#head_img").after('<img id="img_' + keyword + '" src="../' + json.content + '"/>');
				}
...
function change_province(){
	...
	if(province != ''){
		$.get("privilege.php?act=get_regions&pid=" + province, function(data){
			$("#city").empty();
			$("#district").empty();
...
function change_city(){
	var city = $("#city").val();
	if(city != ''){
		$.get("privilege.php?act=get_regions&pid=" + city, function(data){
			$("#district").empty();
...
<body class="gray-bg">
	<div class="wrapper wrapper-content">
		<div class="container no-padding">
			<form action="privilege.php?act=update" method="POST" class="form-horizontal" id="setting_form" target="_self" onsubmit="return make_desc()">
