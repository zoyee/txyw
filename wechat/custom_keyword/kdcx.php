 ...
 *
 */
class kdcx extends keyword_processor {

	function __construct(){
		$this->logger = LoggerManager::getLogger(basename(__FILE__));
		$this->keyword_name = "快递查询";
	}
			...
			if ($ArticleCount >= 1) {
				foreach($orders as $k => $v) {
					...
					if(parent::$oauth_state == 'true'){
						$url = parent::$oauth_location .  parent::$m_url . 'user.php?act=order_tracking&order_id=' . $v['order_id'];
					} else {
						$url = 'user.php?act=order_tracking&order_id=' . $v['order_id'] . '&wxid=' . $fromUsername;
					}
