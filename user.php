...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list','scan_code',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer' , 'face','my_shop_name','my_up_shop_name','my_friend','my_brokerage','my_withdraw','friend_detail');
/* 未登录处理 */
if (empty($_SESSION['user_id']))
{
    if (!in_array($action, $not_login_arr))
    {
        if (in_array($action, $ui_arr))
        {
            ...
            {}*/
        	$oath_user_id = $_REQUEST['oath_user_id'];
        	if($oath_user_id)
        	{
        		$sql = 'SELECT nick_name, user_id FROM ' . $ecs->table('oath_user') . " WHERE id = '$oath_user_id'";
        		$row = $db->getRow($sql);
        		$user->set_session($row['nick_name']);
        		$user->set_cookie($row['nick_name']);
...
if ($action == 'register')
{
    ...
    $logger->debug('二维码注册并登录');
    $affiliate_man = get_affiliate();
    if($affiliate['on']){
    	ini_set('default_socket_timeout', 60);
    	$uuid = create_uuid();
    	$url = $config['site_url']. 'mobile/' . "qrcode.php?act=qr_event&uuid=$uuid&type=2&user_id=$affiliate_man";
		...
		if($json->errcode == 0){
			$qr_path = "mobile/" . $json->data;
			$db->query("insert into " . $GLOBALS['ecs']->table('qr_event') . " (uuid, type, status, dateline,qr_path)values('$uuid', 2, 0, '$dateline', '$qr_path')");
			$smarty->assign('user_qr', $qr_path);
			$smarty->assign('uuid', $uuid);
		}else{
			$smarty->assign('err_msg', '网络不给力，请点击确定刷新页面');
		}
    }
...
$type = empty($_REQUEST['type']) ?  '' : $_REQUEST['type'];
	...
	if($c)
	{
		...
		setcookie("ECS[nickname]",  $info['name'],   $time, $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
		if(!$info['user_id'])
			show_message($c->get_error() , '首页' , $ecs->url() , 'error' , false);


		//$sql = 'SELECT user_name,password,aite_id FROM '.$ecs->table('users').' WHERE aite_id = \''.$info_user_id.'\' OR aite_id=\''.$info['user_id'].'\'';
		$sql = 'SELECT id, user_name,password,o.aite_id,u.user_id,wxid FROM '.$ecs->table('oath_user').' o LEFT JOIN '.$ecs->table('users').' u on o.user_id = u.user_id WHERE o.aite_id = \''.$info['user_id'].'\' and o.type=\''.$type.'\'';
		$count = $db->getRow($sql);
		$oath_user_id = "";
		if(!$count)   // 没有当前数据
		{
			$auto_reg_conf = $db->getRow('select * from wxch_autoreg WHERE `autoreg_id` = 1');
			...
			if(empty($auto_reg_conf['state'])){
				...
				{
					$user_name = $info['name'].'_'.$type.(rand(10000,99999));
				}
				...
				$sql = 'INSERT INTO '.$ecs->table('users').'(user_name , password, aite_id , sex , reg_time , user_rank , is_validated) VALUES '.
						"('$info[name]' , '$user_pass' , '$info_user_id' , '$info[sex]' , '".gmtime()."' , '0' , '1')" ;
				$db->query($sql);
				$user_id = $db -> insert_id();
				$user_name = $name_prefix . $user_id;
				
				$db->autoExecute($GLOBALS['ecs']->table('users'), array(
						'user_name' => $user_name
				), 'UPDATE', " user_id='$user_id'");
				

					$sql = 'INSERT INTO '.$ecs->table('oath_user').'(aite_id , nick_name, reg_time , type , figureurl , sex , user_id) VALUES '.
							"('".$info['user_id']."' , '".$info['name']."' , '".gmtime()."' , '$type' , '".$info['img']."' ,  '0','$user_id')" ;
					$db->query($sql);
					$oath_user_id = $db -> insert_id();
			}
		...
		else
		{
			...
			{ */
			$logger->debug("更新联合登录用户信息" . json_encode($info));
			$sql = 'UPDATE '.$ecs->table('oath_user')." SET nick_name = '$info[name]',figureurl= '".$info['img']."', sex='$info[sex]' WHERE aite_id = '$info[user_id]' and type = '$type'";
			$logger->debug($sql);	
			$db->query($sql);
			//}
		}
		...
		else
		{
			if($count['wxid']){
				...
				ecs_header('Location: '.'http://'.$_SERVER['SERVER_NAME'].'/user.php?act=scan_code&oath_user_id='. $oath_user_id. '&callblock='.$_REQUEST['callblock']);
			}
...
else if($action == 'scan_code'){
	$logger->debug('二维码绑定微信');
    $affiliate_man = get_affiliate();
    $oath_user_id = $_REQUEST['oath_user_id'];
    if($affiliate['on']){
    	ini_set('default_socket_timeout', 60);
    	$uuid = create_uuid();
    	$url = $config['site_url']. 'mobile/' . "qrcode.php?act=qr_event&uuid=$uuid&type=3&user_id=$affiliate_man";
    	$logger->debug('$url = ' . $url);
    	$result = file_get_contents($url);
		$logger->debug('$result = ' . $result);
		$json = json_decode($result);
		...
		if($json->errcode == 0){
			$qr_path = "mobile/" . $json->data;
			$db->query("insert into " . $GLOBALS['ecs']->table('qr_event') . " (uuid, type, status, dateline, oath_user_id, qr_path)values('$uuid', 3, 0, '$dateline', '$oath_user_id', '$qr_path')");
			$smarty->assign('user_qr', $qr_path);
			$smarty->assign('uuid', $uuid);
		}else{
			$smarty->assign('err_msg', '网络不给力，请点击确定刷新页面');
    ...
    $callblock = empty($_REQUEST['callblock']) ? $config['site_url'] : $_REQUEST['callblock'];
    $smarty->assign('back_act', $callblock);
    
	$smarty->assign('template_dir',   $_CFG['template']);
	$url='http://'.$_SERVER['SERVER_NAME'];
	$smarty->assign('site_url',   $url);
	$smarty->display('api_tpl.dwt');
}
...
elseif ($action == 'login')
{
    ...
    if($affiliate['on']){
    	ini_set('default_socket_timeout', 60);
    	$uuid = create_uuid();
    	$url = $config['site_url']. 'mobile/' . "qrcode.php?act=qr_event&uuid=$uuid&type=1&user_id=$affiliate_man";
    	$logger->debug('$url = ' . $url);
    	$result = file_get_contents($url);
		$logger->debug('$result = ' . $result);
		$json = json_decode($result);
		...
		if($json->errcode == 0){
			$qr_path = "mobile/" . $json->data;
			$db->query("insert into " . $GLOBALS['ecs']->table('qr_event') . " (uuid, type, status, dateline, qr_path)values('$uuid', 1, 0, '$dateline', '$qr_path')");
			$smarty->assign('user_qr', $qr_path);
			$smarty->assign('uuid', $uuid);
		}else{
			$smarty->assign('err_msg', '网络不给力，请点击确定刷新页面');
...
elseif ($action == 'my_friend')
{
	...
	for ($i=1;$i<=$num;$i++)
	{
		$sql_one = "select count(*) from ". $ecs->table('users') . "where sales".$i."_id=$user_id";
		$my_friend_num = $GLOBALS['db']->getOne($sql_one);
		if($i==1)
		{
			$num_one = $my_friend_num['count(*)'];
			
		}
		elseif($i==2)
		{
			$num_two = $my_friend_num['count(*)'];
		}
		elseif($i==3)
		{
			$num_three = $my_friend_num['count(*)'];
		}
		elseif($i==4)
		{
			$num_four = $my_friend_num['count(*)'];
		}
		elseif($i==5)
		{
			$num_five = $my_friend_num['count(*)'];
		}
		$res[$i]=$i;
	}
	$smarty->assign('menuarr',       $res);
    $smarty->assign('num_one',$num_one);
    $smarty->assign('num_two',$num_two);
    $smarty->assign('num_three',$num_three);
    $smarty->assign('num_four',$num_four);
    $smarty->assign('num_five',$num_five);
...
/* 分销中心 ： 我的盟友详情页*/
elseif ($action == 'friend_detail')
{
	$smarty->assign('friend_detail', $friend_detail1);
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	$user_list['user_list'] = array();
	$auid = $_SESSION['user_id'];
	$rank = $_GET['rank'];
    $sql_user = "SELECT user_name,user_id FROM " . $ecs->table('users') . " where  sales".$rank."_id=$auid";
    $user_info = $db->getAll($sql_user);
    foreach ($user_info as $key=>$val)
    {
    	$userid = $val['user_id'];
	    $sql_order = "SELECT count(*) FROM " . $ecs->table('order_info') . " where  user_id=$userid and order_status not in (2,4,7)";
	    $order = $db->getOne($sql_order);
	    $user_info[$key]['order_num'] = $order['count(*)'];
    }
	$smarty->assign('user_list',    $user_info);
	$smarty->display('user_transaction.dwt');
